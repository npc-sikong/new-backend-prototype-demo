...
const NOTE_FORMULAS = {
  ...
  '负盈利代理佣金结算': '计算公式：净输赢 = 总输赢 − 场馆费 − 红利 − 返水 + 账户调整 − 存款手续费 − 提款手续费 + 补单输赢；冲正后净输赢 = 净输赢 + 上月结余；佣金 = MAX（0，冲正后净输赢 × 佣金比例 + 佣金调整）；不发放或减少发放金额转入下期结余，原上月结余不回写。',
...
export const PAGE_NOTES = {
  'master:version': note('版本需求归档', '按总控、站点、代理后台集中展示 1.0 与 2.0 版本需求及页面跳转。', '版本、完成时间、模块说明、修改说明、功能验收和页面入口。', '同一版本内仅保留每个业务项的最新说明，未完成的后续增强不计入已完成模块。', '关联三个后台的会员管理、代理列表、团队代理、佣金管理、结算与代理报表。', '2026-07-20 22:41：总控后台新增会员管理与会员锁定流水查询，站点后台和代理后台同步范围保持不变。', {
    updatedAt: UPDATED_AT,
    requirement: '按总控后台、站点后台和代理后台归档最新业务需求；同一版本同一模块只保留最新说明，并提供可用的页面跳转。',
    acceptance: '2.0 版本可看到总控会员锁定流水查询，以及三后台代理资料、佣金、团队关系和角色报表的最新说明与可用入口；代理提款、单线代理管理和会员打码流水统计表不再作为独立模块出现；1.0 归档保持可查询。',
  }),
...
PAGE_NOTES['master:negativeProfit'] = {
  ...
  fields: '展开/收起、代理名称、序号、佣金周期、团队名称、代理编号、代理身份、上级账号、团队人数、下级会员、注册人数、首存人数、活跃人数、新增活跃人数、存款金额、提款金额、总输赢、场馆费、红利、返水、账户调整、存款手续费、提款手续费、补单输赢、净输赢、上月结余、冲正后净输赢、返佣等级、佣金比例、佣金调整、佣金、佣金状态、成为代理时间、加入团队时间、发放人、发放时间、审核人员、审核时间、审核状态、维护人、调整原因和操作；列表底部总计按当前筛选结果逐列展示。',
  logic: '报表默认收起，仅展示团队主记录和单线代理记录；团队主记录前显示“+”，展开后按团队负责人、全部副线顺序展示，成员明细不独立发放。同一团队统一使用团队负责人的代理身份，负责人为官方代理时负责人和全部副线均显示官方代理；展开成员全部人数和金额字段逐列加总必须等于团队主记录，尾差归入最后一条副线。返佣等级位于佣金比例左侧；代理名称固定在左侧、操作固定在右侧。团队主记录和单线代理可确认、不发放或修改发放，修改发放只能减少金额，未发放或减少部分转入下期结余，原上月结余不回写；代理后台仅查看，不可执行发放操作。计算公式：净输赢 = 总输赢 − 场馆费 − 红利 − 返水 + 账户调整 − 存款手续费 − 提款手续费 + 补单输赢；冲正后净输赢 = 净输赢 + 上月结余；佣金 = MAX（0，冲正后净输赢 × 佣金比例 + 佣金调整）。',
...
PAGE_NOTES['master:version'] = {
  ...
  summary: '按总控、站点、代理后台展示最新版本需求；代理后台支持四种身份，并新增独立第五入口“H5代理后台”，以暗夜金融风将现有代理业务适配为移动端体验。',
  fields: '版本、完成时间、后台分组、模块说明、修改说明、功能验收、页面入口、H5代理后台入口、代理身份和移动端验收尺寸。',
  logic: '同一版本同一业务项只保留最新说明，未完成能力不计入已完成验收。H5代理后台位于原“H5 前端”右侧，独立支持团队负责人、副线、单线代理和多层级代理；各身份的模块、功能、筛选、字段、状态、数据口径和操作权限与桌面代理后台对应页面逐项一致，只重排为手机端首页、卡片列表、筛选与详情抽屉、底部导航和横向核对模式。H5身份、最后访问页面和资金演示状态与桌面代理后台隔离，不改变总控后台、站点后台、桌面代理后台和原H5前端。',
  related: '关联代理后台四种身份的代理、会员、资金、报表和活动页面，以及原 H5 前端切换入口；原四门户与原 H5 提现流程保持独立。',
  requirement: '在现有 H5 前端右侧新增完整 H5 代理后台入口，使用暗夜金融视觉重新排版当前代理后台；不得新增、删减或修改桌面代理后台已有功能、筛选、字段、状态、计算口径和操作权限。',
  acceptance: '2.0 版本的代理后台分组可看到 H5代理后台独立模块并直接跳转；四种身份与桌面端逐项功能、字段及权限一致，390×844 和 430×932 尺寸无页面级横向溢出；原四门户与原 H5 前端保持可用且不受影响；1.0 归档保持可查询。',
  boundary: '本次只新增纯前端 H5 代理后台原型和版本说明，不连接真实资金或业务服务，不修改原总控、站点、桌面代理后台及原 H5 前端业务。',
  record: '修改时间：2026-07-21 18:30；修改说明：为代理提供独立、易用的移动经营入口；修改内容：新增第五个“H5代理后台”入口及版本模块卡片，支持四种身份，以暗夜金融风重排现有代理业务，并明确原四门户与原 H5 前端保持不变。',
}

PAGE_NOTES['master:memberLockedFlow'] = note(
  '会员锁定流水查询',
  '在会员管理下按站点、代理和会员查询余额锁定情况，并下钻查看场馆提现流水与充值提现流水的逐项解锁进度。',
  '站点、代理、会员账号或ID、币种、充值额度、总余额、可提现余额、锁定余额、场馆提现流水、充值提现流水；场馆明细包含场馆、锁定额度、目标流水、已完成有效流水、待确认流水、还需解锁流水和状态；充值明细包含顺序、类型、发生时间、计入额度、目标流水、已完成流水、还需解锁流水、状态和盈利解锁额度。',
  '总余额 = 可提现余额 + 锁定余额。场馆提现流水 = 各场馆及通用任务还需解锁流水之和，单项还需解锁流水 = MAX（0，目标流水 − 已完成有效流水），待确认流水不提前释放锁定余额。成功充值与系统发放彩金独立建立流水记录，按发生时间 FIFO 解锁，前一笔未完成时不得跳过；充值或活动盈利金额在本轮所有充值提现流水完成后统一解锁。查询、重置、导出和两类查看明细均为可操作演示。',
  '关联 H5 提现页的可提现余额、锁定余额与解锁条件弹窗，以及会员充值、系统发放彩金和场馆有效流水口径。',
  '2026-07-20 22:41：新增会员锁定流水查询；修改内容：新增会员管理一级菜单、三项筛选、8条会员流水演示数据、查询重置导出、场馆提现流水明细弹窗和充值提现流水明细弹窗。',
  {
    updatedAt: '2026-07-20 22:41',
    requirement: '参照三张图片复制总控会员锁定流水查询的页面内容与两类明细弹窗，统一复用本项目现有筛选、表格、说明和弹窗风格，不额外还原图片中的独立视觉体系。',
    acceptance: '左侧显示会员管理(新)和会员锁定流水查询(新)；筛选、重置、导出可操作；任一列表记录均可打开场馆或充值提现流水明细，member_10086 的金额、状态和四笔 FIFO 记录与参考图一致。',
    boundary: '本页仅复制主列表与两个弹窗原型，使用前端模拟数据，不连接真实会员钱包、投注流水或导出服务；站点后台和代理后台本次不新增该模块。',
  },
)
