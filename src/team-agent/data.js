...
export const INITIAL_STATE = {
  ...LEGACY_STATE,
  agents: [
    agent({ id: '345', account: 'gaodashang', model: '负盈利模式', agentType: '团队代理', teamAgentType: '官方代理', developer: 'gaodashang', boundMemberAccount: 'M100345', email: 'gao@example.com', registeredAt: '2026-05-08 13:20', settlementMode: '团队模式', identity: '主管主线', unit: 'gaodashang01部', lineId: 'LINE-A', effectiveCycle: '2026-07', site: '旺财体育', status: '启用', parent: '无上级代理', subAgents: 8, members: 207, activeMembers: 68, newActiveMembers: 18, depositAmount: 486000, withdrawalAmount: 182000, totalWinLoss: 218000, validBetting: 1640000, plan: '旺财团队月结方案', balance: 79806.45, lastLogin: '2026-07-15 09:22', subAgentDetails: [{ id: '373', account: 'WC002', registeredAt: '2026-06-02 12:30' }, { id: '374', account: 'LGNB', registeredAt: '2026-06-06 17:40' }, { id: '1750', account: 'dailiwc001a', registeredAt: '2026-06-18 09:15' }] }),
...
export const LEGACY_REPORT_ROWS = {
  reversal: [
    ['旺财体育', 'gaodashang', '345', '团队代理', '主线负责人', 4, 170.45, 21687.56, 12000, 2500, 36358.01, 29237.56, 7120.45, 0, 0],
  ...
  returns: [
    ['旺财体育', 'gaodashang', '345', '团队代理', '主线负责人', '余额', '垫付', 6.23, 6.23, 'CZ536', '2026-07-13 11:24:01'],
    ['旺财体育', 'gaodashang', '345', '团队代理', '主线负责人', '余额', '垫付', 0.62, 0.62, 'CZ537', '2026-07-13 11:24:01'],
    ['旺财体育', 'dailiwc001', '1749', '团队代理', '独立代理', '余额', '回款', 94.34, 0, 'CZ492', '2026-07-13 02:02:30'],
    ['旺财体育', 'gaodashang', '345', '团队代理', '主线负责人', '余额', '垫付', 43.61, 43.61, 'CZ524', '2026-07-13 02:02:28'],
...
const NOTE_FORMULAS = {
  ...
  '佣金记录': '计算公式：待发佣金 = 佣金金额 − 已发佣金；代理身份展示规则 = 团队代理显示主线负责人、副线负责人、独立代理或团队成员，星级代理显示几星，层级代理显示几层。',
...
export const PAGE_NOTES = {
  ...
  'master:agents': note('代理管理', '按截图恢复代理管理列表与新增、修改、修改密码操作，在原代理类型基础上补充团队代理及团队类型。', '代理ID、代理账号、代理注册时间、代理类型、团队类型、代理身份、站点编码、上级代理、代理状态、谷歌验证、下属代理、下属会员、佣金方案、代理返佣比例、代理钱包余额和最后登录；新增与修改弹窗保留原代理账号、密码、站点、佣金方案、运营费、上级、状态和备注等基础字段，并在团队代理下展示团队类型和代理身份。', '代理类型保留多层级代理、星级代理，并新增团队代理；列表不再单独展示星级级别字段，原层级级别字段改为代理身份。只有选择团队代理时才展示团队类型和代理身份。团队类型可选官方代理或普通代理，列表仅团队代理显示该字段，非团队代理显示横线；佣金方案同步切换为团队代理返佣方案选项。', '关联团队代理管理、佣金管理、代理佣金结算和代理提款。', '2026-07-17 04:35：代理管理列表去除星级级别字段，将层级级别改名为代理身份，保留团队代理身份和层级代理层级展示。', { updatedAt: '2026-07-17 04:35' }),
  'master:teams': note('团队代理管理', '集中管理代理部、主管主线、副线结构、会员归属、合并业绩、当月结余和团队结算。', '代理部、团队类型、主线、副线、line_id、业务范围、团队人数、会员人数、团队副线/单线、创建与加入时间、新增活跃、新增首存、首存额度、活跃会员、净输赢、上月结余、本月结余调整、冲正后净输赢、佣金比例和状态。', '团队人数、会员人数、团队副线和单线数量均可点击查看明细；团队人数展示代理名称、代理身份和代理ID，会员人数展示会员名称和上级代理，团队副线/单线展示相关代理名称、代理ID和下级会员数。团队详情的主副线结构和业绩考核按每条 line_id 展示新增活跃、新增首存、首存额度、活跃会员和净输赢；新增首存统计本周期首次成功存款会员数，首存额度为这些会员首笔成功存款金额合计。团队合并主线与有效副线范围后统一定级，每周期只生成一张团队账单并只付主线；加入团队当月结余随代理带入，移出时留原团队，解散时剩余结余由指定代理承接。', '关联代理资料、佣金管理、代理佣金结算、内部结算、团队操作记录和模式变更审核。', '2026-07-17 04:41：团队详情的主副线结构和业绩考核在新增活跃右侧新增新增首存、首存额度两个字段，并补齐新建团队和开副线的默认展示值。', { updatedAt: '2026-07-17 04:41' }),
  ...
  'site:teams': note('团队代理管理', '按总控同名页面结构管理旺财体育本站代理部、主副线、成员明细和团队生命周期。', '代理部、团队类型、主管主线、团队人数、会员人数、团队副线/单线、创建时间、加入时间、团队方案、生效周期、新增活跃、新增首存、首存额度、活跃会员、净输赢、本月结余和状态。', '站点后台不展示跨站点选择，只处理旺财体育本站团队。团队人数、会员人数、团队副线和单线数量均可点击查看本站明细；团队详情按每条线路展示新增活跃、新增首存、首存额度、活跃会员和净输赢。加入团队结余带入、移出留原团队、解散由指定代理承接；冻结期间停止新增关系和资金操作。', '关联代理管理、模式变更审核、佣金方案、代理佣金结算、内部结算监控和团队操作记录。', '2026-07-17 05:33：同步总控同名页面字段与明细弹窗，去除副线会员详情字段，并按站点权限固定为本站数据。', { updatedAt: '2026-07-17 05:33' }),
  'site:singles': note('独立单线管理', '按总控同名页面结构管理旺财体育本站直接创建或由副线转入的独立单线。', '线主、来源、推荐人、方案、起始周期、业务范围和状态。', '站点只查看和处理本站独立单线；单人单线初始范围只包含线主节点，后续关系变化从未来完整周期生效。', '关联代理管理、模式变更审核、推荐奖励和代理佣金结算。', '2026-07-17 05:33：统一总控与站点页面名称和业务字段，保留站点创建、加入团队和终止处理权限。', { updatedAt: '2026-07-17 05:33' }),
  ...
  'site:plans': note('佣金方案', '按总控同名页面结构查看和维护旺财体育本站生效的团队、独立单线和推荐奖励方案。', '方案版本、方案类型、生效周期、等级、新增活跃、首充人数、首充额度、活跃会员、总输赢、佣金比例、奖励基数、活跃定义、场馆费和存提款手续费。', '站点只能维护本站方案组合，不可修改其他站点配置；等级中已设置的新增活跃、首充人数、首充额度、活跃会员和总输赢门槛需同时满足，方案变更只影响未来完整周期，历史账单继续使用当期快照。', '关联总控佣金方案、团队代理管理、独立单线管理、代理佣金结算和代理端只读方案。', '2026-07-17 05:33：页面名称同步为佣金方案，并补齐首充人数、首充额度等等级条件和本站权限说明。', { updatedAt: '2026-07-17 05:33' }),
  'site:settlement': note('代理佣金结算', '按总控同名页面结构处理旺财体育本站团队、独立单线和推荐奖励账单。', '代理账号、代理类型、代理身份、账单类型、结算单元、收款人、当月结余、应付、已发、剩余、站点日额度和状态。', '站点列表不展示所属站点列，数据固定为旺财体育；账单审核通过后锁定关系和计算结果，站点可提交审核并按账单剩余与当日剩余额度执行部分发放。', '关联总控代理佣金结算、佣金记录、代理提款和站点额度。', '2026-07-17 05:33：同步总控代理类型和代理身份字段，去除跨站点字段，并保留本站提交、审核和部分发放操作。', { updatedAt: '2026-07-17 05:33' }),
  'site:withdrawals': note('代理提款', '按总控同名页面结构处理旺财体育本站待审核代理提款并查询本站全部提款记录。', '代理账号、上级账号、代理类型、提款类型、订单号、状态、申请时间、USDT金额、实际CNY金额、提款信息和审核信息。', '站点列表不再展示所属站点筛选和字段，数据固定为旺财体育；审核通过扣减代理本人可提现余额，拒绝释放处理中金额，审核结果同步到总控和代理本人记录。', '关联代理管理、代理佣金结算、内部结算、代理钱包和总控代理提款。', '2026-07-17 05:33：统一页面名称为代理提款，去除跨站点筛选和字段，仅保留本站审核、拒绝、完成出款和详情能力。', { updatedAt: '2026-07-17 05:33' }),
  ...
  'agent:bills': note('代理佣金结算', '按总控同名页面口径只读展示当前演示身份本人的平台账单或内部结算记录。', '代理类型、代理身份、账单类型、周期、结算单元、冲正后净输赢、比例、应付、已发、剩余、状态、活跃定义、代理成本和站点方案。', '代理后台不展示所属站点、其他团队或其他代理字段；主管主线和独立代理只读查看本人平台账单，副线负责人不生成平台应付账单，只查看主线向本人的内部结算。', '关联佣金方案、佣金记录、副线内部结算和代理提款。', '2026-07-17 05:33：统一页面名称与总控一致，补充代理类型和代理身份，并按当前身份裁剪为只读本人数据。', { updatedAt: '2026-07-17 05:33' }),
  ...
  'agent:withdrawals': note('代理提款', '按总控同名页面口径展示当前演示身份本人的可提现余额、处理中金额和提款申请。', '代理身份、账号、可提现余额、处理中金额、提款类型、收款网络、USDT金额、实际CNY金额、提款信息、订单状态、申请时间和审核信息。', '代理端不展示站点审核操作、其他代理或跨站点字段；申请金额只能来自本人可提现余额，待审核和处理中订单占用余额，三种身份只查看本人订单。', '关联代理佣金结算、团队内部结算、代理钱包及站点/总控提款审核。', '2026-07-17 05:33：页面名称同步为代理提款，保留当前身份本人申请和订单查询，去除审核与跨代理权限。', { updatedAt: '2026-07-17 05:33' }),
...
function reportNote(portal, page, spec) {
  ...
  const roleLogic = isAgent
    ? '代理后台只展示当前演示身份本人及授权下级范围，不展示所属站点、其他团队或其他线路；主管主线可查看本人团队及各线汇总，副线只查看本人线路，独立代理只查看独立单线。'
...
PAGE_NOTES['site:siteAgents'] = note('代理管理', '按总控同名页面结构查看旺财体育本站代理主档，并补充团队结算信息。', '代理ID、账号、代理类型、代理模型、状态、推广人员、上级代理、结算模式、代理身份、代理部、line_id、负责人、生效周期、当月结余、会员、投注和余额。', `${RELATION_LOGIC} 页面固定为旺财体育本站，不展示跨站点字段；站点可查询本站代理资料并进入团队或独立单线管理，代理资料修改仍由总控处理，不能操作其他站点代理。 ${BALANCE_LOGIC}`, '关联团队代理管理、独立单线管理、模式变更审核、代理财务和代理端代理管理。', '2026-07-17 05:33：同步总控代理管理名称与团队代理字段，并按站点权限固定为本站资料只读查询。', { updatedAt: '2026-07-17 05:33' })
...
PAGE_NOTES['agent:personalCommission'] = note('代理端个人佣金', '按当前身份查看本人佣金、内部结算到账、待到账和可用余额。', '身份、账号、结算单元、line_id、周期、当月结余、比例、应付或结算金额、已到账、待到账、状态和代理余额。', `${BALANCE_LOGIC} 副线只显示主线向本人支付的内部结算，不形成平台对副线的佣金账单。`, '关联佣金方案、代理佣金结算、内部结算、账变明细、代理代存和代理提款。', '2026-07-15 18:30：恢复个人佣金汇总，补充三身份资金边界、筛选、详情和导出。')
...
PAGE_NOTES['master:agents'] = {
  ...
  record: '修改时间：2026-07-17 04:35；修改说明：简化代理管理列表字段，避免星级和层级字段并列造成理解成本；修改内容：列表去除星级级别字段，原层级级别字段改名为代理身份，用于展示团队代理身份或层级代理层级，星级代理该列显示横线。',
}
...
PAGE_NOTES['master:returns'] = {
  ...
  logic: '代理类型分为团队代理、星级代理和层级代理；代理身份按类型展示，团队代理显示主线负责人、副线负责人、独立代理或团队成员，星级代理显示几星，层级代理显示几层；原代理等级字段不再单独展示。回款只冲减原冲正台账，不直接改变团队主副线关系；计算公式：额度缺口 = 同一冲正账目原欠款额度 − 累计成功回款金额；本次记录为“垫付”时增加待回款额度，为“回款”时减少额度缺口。',
