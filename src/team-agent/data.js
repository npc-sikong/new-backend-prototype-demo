...
export const PAGE_NOTES = {
  'master:version': note('版本需求归档', '按总控、站点、代理后台集中展示 1.0 与 2.0 版本需求及页面跳转。', '版本、完成时间、模块说明、修改说明、功能验收和页面入口。', '同一版本内仅保留每个业务项的最新说明，未完成的后续增强不计入已完成模块。', '关联三个后台的代理列表、团队代理、佣金管理、结算与代理报表。', '2026-07-18 16:11：移除会员管理版本项，并覆盖更新站点后台和代理后台按总控当前代理页面直接同步的说明。', {
    updatedAt: UPDATED_AT,
    requirement: '按总控后台、站点后台和代理后台归档最新业务需求；同一版本同一模块只保留最新说明，并提供可用的页面跳转。',
    acceptance: '2.0 版本可按三后台看到代理资料、佣金、团队关系和角色报表的最新说明与可用入口；会员管理、代理提款和单线代理管理不再作为独立模块出现；1.0 归档保持可查询。',
  }),
  ...
  'master:negativeProfit': note('负盈利代理报表', '在代理管理下新增负盈利代理报表，按佣金周期集中查看负盈利模式代理及负向结余账单的经营、成本、结余、佣金和审核发放结果。', '序号、佣金周期、团队名称、代理编号、代理账号、代理身份、上级账号、团队人数、下级人数、注册人数、首存人数、活跃人数、新增活跃人数、存款金额、提款金额、总输赢、场馆费、红利、返水、账户调整、存款手续费、提款手续费、补单输赢、净输赢、上月结余、冲正后净输赢、佣金比例、佣金调整、佣金、佣金状态、成为代理时间、加入团队时间、发放人、发放时间、审核人员、审核时间、审核状态、维护人和调整原因；列表底部总计按负盈利代理明细字段逐列展示当前筛选结果的字段总计。', '报表按佣金周期、代理身份、佣金状态、审核状态、代理或团队关键字查询；代理身份只显示官方代理或普通代理。字段筛选支持多选负盈利代理明细中的全部字段，运营可按当前核对重点勾选任意列组合，宽表的左右拉动条固定在负盈利代理明细列表下方，不再撑到页面底部。报表展示负盈利模式代理及冲正后净输赢为负的账单记录；总计是负盈利代理明细的字段总计，随字段筛选逐列展示当前字段，数值字段按当前筛选结果求和，非数值字段用于定位或显示横线，不随分页切换变化；净输赢按总输赢扣除场馆费、红利、返水和手续费后计入账户调整与补单输赢；冲正后净输赢叠加上月结余；佣金按冲正后净输赢乘佣金比例再加佣金调整且不小于零。', '关联代理列表、团队代理管理、代理佣金结算、佣金记录和代理收益看板。', '2026-07-18 22:20：统一报表代理身份字段；修改内容：团队类型列和筛选改为代理身份，报表统一显示官方代理或普通代理。', { updatedAt: '2026-07-18 22:20' }),
  'master:agentOperations': note('代理操作记录', '在代理管理下新增代理操作记录，用于集中回放代理团队创建、副线调整、关系变更和团队状态调整等操作流水。', '筛选字段包含团队名称、代理身份、代理编号、代理账号和创建时间；列表字段包含序号、团队名称、代理身份、主线编号、主线账号、副线账号、操作内容、操作理由、操作时间和操作人。', '团队名称和代理身份使用下拉筛选；代理身份只显示官方代理或普通代理。代理编号匹配主线编号，代理账号同时匹配主线账号和副线账号；创建时间按起止日期筛选。副线账号可包含多个账号，列表中按标签排列展示，便于运营快速识别本次操作影响的副线范围。', '关联代理列表、团队代理管理、模式变更审核、关系与模式申请和修改代理关系记录。', '2026-07-18 22:20：统一报表代理身份字段；修改内容：团队类型列和筛选改为代理身份，操作记录统一显示官方代理或普通代理。', { updatedAt: '2026-07-18 22:20' }),
...
const SITE_SYNCED_NOTES = {
  agents: 'site:siteAgents', negativeProfit: 'master:negativeProfit', agentOperations: 'master:agentOperations', teams: 'site:teams', settlement: 'site:settlement', records: 'site:commissionRecords', reversal: 'site:reversal', returns: 'site:returns', cycle: 'site:cycle',
}
const AGENT_SYNCED_NOTES = {
  agents: 'agent:downline', negativeProfit: 'master:negativeProfit', agentOperations: 'master:agentOperations', teams: 'agent:dashboard', reversal: 'agent:reversal', returns: 'agent:returns',
}
