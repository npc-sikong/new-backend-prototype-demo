...
export const PAGE_NOTES = {
  ...
  h5: note('H5 提现额度与解锁条件说明', '顶部展示中心钱包、锁定钱包和福利中心；页面保持 H5 手机端窄屏样式，选择提现方式后展开账户、金额、资金密码和解锁条件。', '中心钱包、锁定钱包、福利中心、一键回收、隐藏无余额场馆、提现方式、提现账户、提现金额、可提现、锁定、解锁条件、流水类型、手续费、实际到账、最低提现额度和资金密码。', '提现金额不得超过当前可提现金额；可提现与锁定金额展示在提现金额标题下方、输入框上方，文案简化为“可提现 / 锁定”且不展示单位。“解锁条件”弹窗合并展示场馆流水、充值流水和彩金流水，只展示仍需解锁的记录，不展示已解锁记录和盈利解锁额度行；第一列直接显示名称：场馆显示场馆名称，充值显示“充值”，彩金显示具体名称如 VIP周礼金、返水或活动奖励。弹窗底部提示投注流水同步可能存在延迟，如当前解锁额度与实际情况不符，请稍后刷新并重新查看。', '关联会员打码流水统计表、会员充值、会员钱包、场馆流水和会员取款方式。', '2026-07-16 16:48：更新解锁条件弹窗底部延迟同步提示文案。', { updatedAt: '2026-07-16 16:48' }),
}
...
Object.entries(NOTE_COMPARISONS).forEach(([key, comparison]) => {
  ...
  PAGE_NOTES[key] = {
    ...current,
    comparison,
    updatedAt: NOTE_COMPARISON_UPDATED_AT,
    record: `修改时间：${NOTE_COMPARISON_UPDATED_AT}；修改说明：补全本页相对原后台的业务差异说明；修改内容：保留原后台已有能力说明，并按字段、类型、筛选、页签或弹窗、操作功能和业务规则逐项标明本原型新增或修改内容。`,
  }
})
