...
export const PAGE_NOTES = {
  ...
  'master:negativeProfitModeGuide': note(
    ...
    '返佣等级、佣金比例、有效活跃人数、新增活跃人数、投注总盈亏、团队负责人、副线、代理身份、上月结余、当月净利润、代理佣金、充值提现流水和场馆活动提现流水。',
    '返佣比例按结算周期实际运营数据命中等级，不固定发放；未达到最低等级时本期收益为0，盈亏以结余带到下期。团队代理按全团队汇总数据定级，官方代理与普通代理不可混组；单线代理独立定级，上级代理为推荐人。负盈利模式只有一个层级，不产生级差佣金或级差运营手续费分摊，不使用垫付和回款。代理佣金 = 当月净利润 × 代理佣金比例；当月净利润 = 结算周期内会员给平台带来的盈利 − 三方场馆费用 − 运营费用 + 上月结余。充值提现流水与场馆活动提现流水按资金使用场景分别扣减，互不混算。',
...
PAGE_NOTES['master:version'] = {
  ...
  logic: '同一版本同一业务项只保留最新说明，未完成能力不计入已完成验收。总控后台负盈利模式说明用于解释按运营数据定级、团队与单线代理规则、上月结余、佣金计算，以及充值与场馆活动提现流水分别扣减的口径。H5代理后台位于原“H5 前端”右侧：首页直接完整展开当前身份全部模块，不再重复展示财务、会员、报表和全部功能快捷区；团队负责人、副线和单线代理各同步桌面端11个业务模块，多层级代理同步12个业务模块，只允许移动端重排，不新增或删减字段和功能。',
