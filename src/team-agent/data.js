...
export const PAGE_NOTES = {
  ...
  'master:negativeProfitModeGuide': note(
    '负盈利模式说明',
    '集中说明充值、彩金、锁定余额与提现投注流水的逐笔统计和解锁顺序。',
    '资金记录顺序、充值或彩金类型、充值额度、单笔目标流水、单笔已完成有效流水、单笔还需提现投注流水、锁定余额和可提现余额。',
    '存在锁定余额时不能全额提现，只能提取已完成流水记录中尚未亏损的充值本金。每笔充值或彩金独立记录额度与提现投注流水，多笔记录按发生时间 FIFO 解锁；完成一笔只释放该笔可释放本金，本轮全部记录完成后才统一释放盈利。单笔还需提现投注流水 = MAX（0，单笔目标流水 − 单笔已完成有效流水）；单笔可转本金不超过该笔充值额度，并受用户当前剩余余额限制。全部余额转为可提现后本轮结束，之后再次充值重新计为第一笔记录。',
    '关联会员提现流水查询、H5提现页的锁定余额与解锁条件、会员充值、活动彩金和场馆有效流水。',
    '2026-07-21 19:10：新增负盈利模式业务说明页面；修改内容：新增逐笔统计、FIFO解锁、本金释放、盈利统一解锁及两类特殊场景说明。',
    {
      updatedAt: '2026-07-21 19:10',
      requirement: '在版本需求说明下新增负盈利模式说明，完整解释多笔充值或彩金的独立流水、顺序解锁、本金释放、盈利锁定和亏损后的余额处理。',
      acceptance: '左侧菜单可进入独立说明页；页面完整展示核心限制、五步解锁顺序、四项关键规则、计算口径及两类特殊场景；站点后台、代理后台和H5页面不新增该菜单。',
      boundary: '本页仅作为产品与运营业务规则说明，不新增查询、资金操作或真实流水处理能力。',
    },
  ),
...
PAGE_NOTES['master:version'] = {
  ...
  logic: '同一版本同一业务项只保留最新说明，未完成能力不计入已完成验收。总控后台新增负盈利模式说明，集中解释每笔充值或彩金的独立提现投注流水、FIFO解锁、本金释放和盈利统一解锁规则。H5代理后台位于原“H5 前端”右侧，独立支持团队负责人、副线、单线代理和多层级代理；各身份的业务功能与桌面代理后台一致。',
  related: '关联负盈利模式说明、会员提现流水查询、H5提现解锁条件、代理后台四种身份页面和 H5代理后台入口。',
  requirement: '归档负盈利模式说明及现有各后台最新需求；从版本卡片可跳转到对应页面，同一模块不堆叠重复修改历史。',
  acceptance: '2.0版本总控后台分组可看到负盈利模式说明并直接跳转；说明页包含逐笔流水、FIFO、本金与盈利解锁及两类特殊情况；代理后台分组仍可看到H5代理后台，原门户保持可用。',
  boundary: '版本页只归档当前原型已实现的业务页面与说明，不连接真实资金或业务服务。',
  record: '修改时间：2026-07-21 19:10；修改说明：补充负盈利模式业务规则的统一入口；修改内容：版本说明新增负盈利模式说明模块卡片和页面跳转，并保留H5代理后台及其他既有版本内容。',
}
