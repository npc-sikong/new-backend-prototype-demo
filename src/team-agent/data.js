...
const NOTE_FORMULAS = {
  ...
  '佣金记录': '计算公式：待发佣金 = 佣金金额 − 已发佣金；代理身份展示规则 = 团队代理显示团队负责人、副线或独立代理，星级代理显示几星，层级代理显示几层。',
...
export const PAGE_NOTES = {
  ...
  'master:agents': note('代理列表', '在“代理管理”大模块下展示代理列表与新增、修改、修改密码操作，在原代理类型基础上补充团队代理及团队类型。', '代理ID、代理账号、代理注册时间、代理类型、团队类型、代理身份、站点编码、上级代理、代理状态、谷歌验证、下属代理、下属会员、佣金方案、代理返佣比例、代理钱包余额和最后登录；新增与修改弹窗保留原代理账号、密码、站点、佣金方案、运营费、上级、状态和备注等基础字段，并在团队代理下展示团队类型和代理身份。', '代理类型保留多层级代理、星级代理，并新增团队代理；列表不再单独展示星级级别字段，原层级级别字段改为代理身份。新增代理选择团队代理时，代理身份固定为团队负责人；修改代理时可继续维护团队负责人、副线和独立代理。团队类型可选官方代理或普通代理，列表仅团队代理显示该字段，非团队代理显示横线；佣金方案同步切换为团队代理返佣方案选项。', '关联团队代理管理、佣金方案、代理佣金结算和佣金记录。', '2026-07-17 19:45：代理管理大模块下的当前页面名称恢复为“代理列表”；修改内容：顶部页签、页面标题、导出提示和业务说明标题使用代理列表，左侧父级大模块仍保持代理管理。', { updatedAt: '2026-07-17 19:45' }),
  'master:teams': note('团队代理管理', '集中管理代理部、团队负责人、团队成员列表、会员归属、合并业绩、团队当前余额和团队业绩查看。', '代理部、团队类型、主线、副线、line_id、团队成员、会员人数、团队代理总人数、总会员数、活跃会员数、副线数、独立代理数、团队当前余额、未结算收益、当前返佣星级、团队返佣等级、距离下一等级条件、代理名称、统计日期、活跃会员、新增活跃、新增首存、首存额度、总盈亏、运营费用、净收益、团队贡献占比、总计、状态和操作。', '团队列表去除顶部四个汇总标签和团队副线/单线列，保留团队成员与会员人数下钻；团队成员展示代理名称、代理身份和代理ID，会员人数展示会员名称和上级代理。列表操作中的开副线只打开新增副线弹窗，不切换到团队详情。团队详情的团队概况顶部展示团队当前余额与未结算收益，不再展示主线团队可用余额；当前返佣星级作为团队概况主体展示，用大号等级和返佣比例突出当前命中结果，下一等级所需条件作为副体展示新增活跃、新增首存、首存额度、活跃会员和团队当前余额差距。团队详情的“主副线结构”改为“团队成员列表”，按每条 line_id 展示代理名称、活跃会员、新增活跃、新增首存、首存额度和总盈亏，不再展示显式业务范围；活跃会员和新增活跃人数可点击查看该代理本人直属会员明细，不展示其他副线或团队汇总会员。团队成员列表在状态右侧新增操作字段，支持停用当前成员和更换身份；更换身份弹窗只允许选择独立代理或团队负责人，保存后更新当前演示行身份与状态。团队详情的“团队分佣结算”改为“团队业绩查看”，去除上方四个大标签和历史结算记录，仅保留业绩列表；可按代理名称、身份、line_id 和统计日期查询，并可导出当前筛选结果；列表去除本期预估分红、发放状态、操作、历史欠款和代理结余字段，列表下方展示当前筛选结果总计；分页和每页条数可切换。新增首存统计本周期首次成功存款会员数，首存额度为这些会员首笔成功存款金额合计；总盈亏 = 各线路总输赢合计，净收益 = 总盈亏 − 运营费用。团队合并主线与有效副线范围后统一定级，每周期只生成一张团队账单并只付主线；加入团队当月结余随代理带入，移出时留原团队，解散时剩余结余由指定代理承接。', '关联代理资料、佣金管理、代理佣金结算、团队账单、团队操作记录和模式变更审核。', '2026-07-17 18:33：团队成员列表新增成员操作；修改内容：状态右侧新增操作字段，提供停用和更换身份，更换身份弹窗可将代理调整为独立代理或团队负责人。', { updatedAt: '2026-07-17 18:33' }),
  'master:plans': note('佣金方案', '按原返佣方案页面维护方案列表，在原配置中支持新增层级、星级和团队代理方案，并新增“推荐奖励”切页维护推荐奖励模板及站点应用关系。', '代理返佣方案字段包含序号、返佣方案名称、方案详情、创建时间、最后操作人、操作时间和操作；推荐奖励字段包含推荐奖励模板名称、奖励层级、已配置站点、生效周期、状态、最后操作人和操作；新增和修改推荐奖励模板弹窗支持模板名称、生效周期、状态、多个奖励层级、推荐人员净收益门槛和推荐人提成百分比；配置站点弹窗用于把模板应用到一个或多个站点，并提示当前站点已绑定的模板。', '团队佣金方案作为原返佣方案中的一条配置，不单独新增方案模块；新增代理方案时可选择层级代理、星级代理或团队代理。层级代理和星级代理只维护对应层级或星级的返佣比例；团队代理可维护新增活跃、活跃会员和总输赢门槛，活跃会员和新增活跃均按当前方案绑定的充值金额或有效投注门槛判定。推荐奖励先维护一套或多套模板，模板内可配置多个层级；站点只选择已维护好的模板，不单独维护一套比例；同一站点同一时间只能绑定一个推荐奖励模板，保存当前模板配置时，重复站点会从其它模板自动解绑。计算公式：命中奖励 = 推荐人员净收益 × 当前命中层级提成百分比。', '关联代理列表、团队代理管理、代理佣金结算、推荐奖励账单和站点佣金方案。', '2026-07-17 20:23：推荐奖励配置增加站点唯一性规则；修改内容：每个站点同一时间只能绑定一个推荐奖励模板，配置站点弹窗展示当前占用模板，保存后自动从其它模板解绑重复站点。', { updatedAt: '2026-07-17 20:23' }),
  'master:settlement': note('代理佣金结算', '按原后台代理佣金结算未发放区域重构列表，用代理类型和代理身份替代原星级、层级两列，并补充团队负责人发放控制。', '代理名称、所属站点、代理类型、代理身份、返佣比例、账单类型、直属上级代理、账期范围、代理直属会员盈亏、本月收益、本月结余和操作。', '代理类型统一为团队代理、星级代理和层级代理；本页团队代理身份不再展示副线，改为团队负责人、团队成员或独立代理；星级代理显示几星，层级代理显示几层。团队负责人可选择不发放或修改发放金额，不发放或减少发放的金额记入本月结余并随下个周期一起统计；0收益代理可一键全部确认，不影响有收益账单。站点账单不属于代理身份，使用横线占位。', '关联佣金方案、代理列表、团队代理管理、佣金记录和站点账单发放。', '2026-07-17 17:17：代理佣金结算页去除代理身份中的副线展示，团队负责人增加不发放和修改发放，新增一键结算0收益代理，并展示本月结余记账金额。', { updatedAt: '2026-07-17 17:17' }),
  'master:records': note('佣金记录', '按原后台佣金发放记录样式展示佣金周期、代理账号、站点名称、代理类型、代理身份、金额、方案、状态和发放信息。', '佣金周期、代理账号、站点名称、代理类型、代理身份、总输赢、月流水、首充金额、留存天数、佣金金额、返佣方案、佣金状态、发放时间、发放人员和详情。', '代理类型分为团队代理、星级代理和层级代理；团队代理身份展示团队负责人、副线或独立代理，星级代理展示几星，层级代理展示几层。记录详情沿用同一身份口径，不再在佣金记录列表中展示单独星级或层级字段。', '关联代理佣金结算、团队账单、独立单线账单、推荐奖励和主线可用余额。', '2026-07-17 16:44：佣金记录中的团队代理身份统一为团队负责人、副线和独立代理。', { updatedAt: '2026-07-17 16:44' }),
  'master:reversal': note('冲正统计报表', '按截图重构冲正统计详情页，用原报表密度统计代理冲正业务数据，并补充代理类型与代理身份识别。', '所属站点、代理账号、ID、代理类型、代理身份、垫付冲正代理总人数、垫付余额、垫付级差佣金、垫付会员盈利、垫付直属佣金、垫付总计、回款总计、垫付剩余金额、欠款人数和欠款总计。', '代理类型分为团队代理、星级代理和层级代理；团队代理身份展示团队负责人、副线或独立代理，星级代理展示几星，层级代理展示几层。垫付总计 = 垫付余额 + 垫付级差佣金 + 垫付会员盈利 + 垫付直属佣金；垫付剩余金额 = 垫付总计 − 回款总计；欠款总计用于统计仍需追回的欠款额度。', '关联佣金记录、团队账单、代理列表和冲正回款报表。', '2026-07-17 16:44：冲正统计中的团队代理身份统一为团队负责人、副线和独立代理。', { updatedAt: '2026-07-17 16:44' }),
  'master:returns': note('冲正回款报表', '按原回款明细样式重构列表，并在 ID 右侧补充代理类型与代理身份。', '所属站点、名称、ID、代理类型、代理身份、类型、垫付或回款、额度、额度缺口、冲正账目ID和时间。', '代理类型分为团队代理、星级代理和层级代理；代理身份按类型展示，团队代理显示团队负责人、副线或独立代理，星级代理显示几星，层级代理显示几层；原代理等级字段不再单独展示。回款只冲减原冲正台账，不直接改变团队主副线关系。', '关联冲正统计、佣金账单和代理余额。', '2026-07-17 16:44：冲正回款中的团队代理身份统一为团队负责人、副线和独立代理。', { updatedAt: '2026-07-17 16:44' }),
  ...
  'site:plans': note('佣金方案', '按总控同名页面结构查看和维护旺财体育本站生效的团队、独立单线和推荐奖励方案。', '方案版本、方案类型、生效周期、等级、新增活跃、活跃会员、总输赢、佣金比例、奖励基数、活跃会员判定条件、新增活跃判定条件、场馆费和存提款手续费。', '站点只能维护本站方案组合，不可修改其他站点配置；层级代理和星级代理只维护返佣比例；团队等级中已设置的新增活跃、活跃会员和总输赢门槛需同时满足，活跃会员与新增活跃判定条件跟随方案生效，方案变更只影响未来完整周期，历史账单继续使用当期快照。', '关联总控佣金方案、团队代理管理、代理佣金结算和代理端只读方案。', '2026-07-17 16:56：同步总控佣金方案新口径，去除首充人数和首充额度，团队方案的活跃会员和新增活跃判定条件改为随方案维护。', { updatedAt: '2026-07-17 16:56' }),
  ...
  'agent:bills': note('代理佣金结算', '按总控同名页面口径只读展示当前演示身份本人的平台账单或内部结算记录。', '代理类型、代理身份、账单类型、周期、结算单元、冲正后净输赢、比例、应付、已发、剩余、状态、活跃定义、代理成本和站点方案。', '代理后台不展示所属站点、其他团队或其他代理字段；团队负责人和独立代理只读查看本人平台账单，副线不生成平台应付账单，只查看主线向本人的内部结算。', '关联佣金方案、佣金记录和副线内部结算。', '2026-07-17 05:33：统一页面名称与总控一致，补充代理类型和代理身份，并按当前身份裁剪为只读本人数据。', { updatedAt: '2026-07-17 05:33' }),
...
PAGE_NOTES['site:siteAgents'] = note('代理列表', '按总控同名页面结构查看旺财体育本站代理主档，并补充团队结算信息。', '代理ID、账号、代理类型、代理模型、状态、推广人员、上级代理、结算模式、代理身份、代理部、line_id、负责人、生效周期、当月结余、会员、投注和余额。', `${RELATION_LOGIC} 页面固定为旺财体育本站，不展示跨站点字段；站点可查询本站代理资料，团队操作统一进入团队代理管理，代理资料修改仍由总控处理，不能操作其他站点代理。 ${BALANCE_LOGIC}`, '关联团队代理管理、模式变更审核、代理财务和代理端代理列表。', '2026-07-17 17:39：将原“代理管理”模块名称统一改为“代理列表”，页面结构、字段和操作保持不变。', { updatedAt: '2026-07-17 17:39' })
...
PAGE_NOTES['agent:downline'] = note('代理列表', '按总控同名页面口径查看当前演示身份本人及授权下级的代理树、经营归属和关系版本。', '关系层级、代理ID、账号、上级、代理模型、结算模式、代理身份、结算单元、line_id、生效周期、会员、活跃会员和状态。', `${RELATION_LOGIC} 代理后台不展示所属站点、跨团队管理或其他代理操作；团队负责人查看本人团队范围，副线和独立代理只查看本人线路。`, '关联代理会员、关系与模式申请、个人财务和站点代理列表。', '2026-07-17 17:39：将原“代理管理”模块名称统一改为“代理列表”，页面结构、字段和操作保持不变。', { updatedAt: '2026-07-17 17:39' })
PAGE_NOTES['agent:readonlyPlans'] = note('佣金方案', '按总控同名页面口径只读查看当前身份适用的团队、独立单线和推荐奖励方案。', '方案编号、名称、类型、生效周期、等级、新增活跃、活跃会员、总输赢、佣金比例、奖励比例、活跃会员判定条件、新增活跃判定条件和状态。', `代理端不展示所属站点和修改操作，只能查看当前身份适用方案；层级代理和星级代理只展示返佣比例，团队方案展示新增活跃、活跃会员、总输赢以及随方案绑定的活跃判定条件；方案变更只影响未来完整周期，历史账单使用生成时快照。 ${BALANCE_LOGIC}`, '关联站点佣金方案、个人佣金、代理佣金结算和代理财务。', '2026-07-17 16:56：代理端只读方案同步总控新口径，去除首充人数和首充额度，返佣比例按百分比展示。', { updatedAt: '2026-07-17 16:56' })
...
PAGE_NOTES['agent:dashboard'] = note('代理运营看板', '将团队经营看板兼容为完整代理运营首页，按团队负责人、副线和独立线主展示经营、资金与团队操作。', '身份、代理账号、充值、提款、投注、有效投注、活跃会员、净输赢、当月结余、等级、佣金、到账、余额、团队线路和状态。', `${BALANCE_LOGIC} 团队负责人查看团队汇总与各线，副线只查看本人线路且不显示平台账单和团队余额，独立线主只查看独立单线。`, '关联下级代理、代理会员、个人财务、佣金账单、内部结算和关系申请。', '2026-07-17 16:44：代理后台演示身份统一为团队负责人、副线和独立线主，并补充副线权限边界。', { updatedAt: '2026-07-17 16:44' })
...
PAGE_NOTES['master:agents'] = {
  ...
  record: '修改时间：2026-07-17 16:44；修改说明：统一副线身份口径，避免把副线误解为单独负责人；修改内容：所有模块的代理身份统一为团队负责人、副线和独立代理，并补充副线为团队内成员身份的业务说明。',
}
...
PAGE_NOTES['master:returns'] = {
  ...
  logic: `代理类型分为团队代理、星级代理和层级代理；代理身份按类型展示，团队代理显示团队负责人、副线或独立代理，星级代理显示几星，层级代理显示几层；原代理等级字段不再单独展示。回款只冲减原冲正台账，不直接改变团队主副线关系；计算公式：额度缺口 = 同一冲正账目原欠款额度 − 累计成功回款金额；本次记录为“垫付”时增加待回款额度，为“回款”时减少额度缺口；${SECONDARY_ROLE_NOTE}`,
  record: '修改时间：2026-07-17 16:44；修改说明：冲正回款报表同步统一副线身份口径；修改内容：团队代理身份只展示团队负责人、副线和独立代理，并补充副线为团队内成员身份的说明。',
}
