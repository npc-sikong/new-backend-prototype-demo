...
export const PAGE_NOTES = {
  'master:version': note('版本需求归档', '按总控、站点、代理后台集中展示 1.0 与 2.0 版本需求及页面跳转。', '版本、完成时间、模块说明、修改说明、功能验收和页面入口。', '同一版本内仅保留每个业务项的最新说明，未完成的后续增强不计入已完成模块。', '关联三个后台的会员管理、代理列表、团队代理、佣金管理、结算与代理报表。', '2026-07-21 12:32：总控后台会员锁定流水查询改名为会员提现流水查询，并补充历史锁定、解锁及导出能力。', {
    updatedAt: UPDATED_AT,
...
PAGE_NOTES['master:memberLockedFlow'] = note(
  '会员提现流水查询',
  '在会员管理下按站点、代理和会员查询充值额度、未解锁彩金及余额锁定情况，并下钻查看当前与历史提现流水。',
  '站点、代理、会员账号或ID、币种、充值额度、未解锁彩金、总余额、可提现余额、锁定余额、充值/彩金提现流水；明细包含开始时间、类型、锁定额度、目标流水、已完成有效流水、待确认流水、还需解锁流水、解锁时间和状态。',
  '总余额 = 可提现余额 + 锁定余额。未解锁彩金为当前仍受提现流水约束的彩金金额；提现流水列表同时展示锁定中与历史已解锁记录，同一时间最多存在一条场馆活动，其余记录按具体活动彩金名称或充值展示。已解锁记录保留开始时间、解锁时间且还需解锁流水为0；弹窗导出生成当前会员的完整提现流水文件。',
  '关联 H5 提现页的可提现余额、锁定余额与解锁条件弹窗，以及会员充值、系统发放彩金和场馆有效流水口径。',
  '2026-07-21 12:32：强化会员提现流水历史核对；修改内容：模块改名为会员提现流水查询，弹窗提现流水列表增加历史已解锁记录、解锁时间和实际导出按钮。',
  {
    updatedAt: '2026-07-21 12:32',
    requirement: '将会员锁定流水查询改名为会员提现流水查询；充值/彩金提现流水弹窗需同时展示当前锁定和历史解锁记录，并支持导出完整提现流水。',
    acceptance: '菜单、面包屑、页面标题和说明统一显示会员提现流水查询；member_10086 弹窗同时显示锁定中与已解锁记录，历史记录带解锁时间且剩余流水为0；点击导出可下载CSV文件并显示操作反馈。',
