...
function getTeamType(team) {
  return team?.teamAgentType || team?.teamType || '普通代理'
}
...
function SiteTeamsPage({ onToast }) {
  ...
  const [teamForm, setTeamForm] = useState({ name: '', mainAgent: '', site: SITE_NAME, plan: 'DW负盈利佣金方案', startCycle: '2026-08', teamType: '普通代理' })
  ...
  const [preferenceForm, setPreferenceForm] = useState({ teamType: '普通代理' })
  ...
  const columns = [
    { key: 'code', label: '代理部编号' }, { key: 'name', label: '代理部名称', render: (value) => <b className="ta-primary-text">{value}</b> }, { key: 'teamType', label: '代理类型', render: (_, row) => getTeamType(row) }, recommenderColumn(), { key: 'mainAgent', label: '团队负责人' },
  ...
  const operationColumns = [
    ...
    { key: 'teamType', label: '代理类型', render: (value) => value || '—' },
    recommenderColumn(),
  ...
  return <>
    ...
    <Modal open={modal === 'create'} title="创建代理部" description="站点指定主线、团队方案和未来完整生效周期。" onClose={close} onConfirm={() => resultMessage(createTeam(teamForm), onToast, close)}>
      <FormGrid><Field label="代理部名称" required help="建议使用“发展人拼音+序号部”，仅作命名提示，不强制校验。"><Input value={teamForm.name} onChange={(value) => setTeamForm({ ...teamForm, name: value })} placeholder="例如：zhangsan1部" /></Field><Field label="团队负责人" required><Input value={teamForm.mainAgent} onChange={(value) => setTeamForm({ ...teamForm, mainAgent: value })} placeholder="代理账号" /></Field><Field label="代理类型"><Select value={teamForm.teamType} onChange={(value) => setTeamForm({ ...teamForm, teamType: value })} options={['官方代理', '普通代理']} /></Field><Field label="团队方案"><Select value={teamForm.plan} onChange={(value) => setTeamForm({ ...teamForm, plan: value })} options={data.plans.filter((plan) => plan.type === '团队佣金方案').map((plan) => plan.name)} /></Field><Field label="生效周期"><Select value={teamForm.startCycle} onChange={(value) => setTeamForm({ ...teamForm, startCycle: value })} options={['2026-08', '2026-09']} /></Field></FormGrid>
    </Modal>
    <Modal open={modal === 'secondary'} title={`为 ${team?.name || ''} 开设副线`} description="站点直接复核并创建明确 line_id 和业务范围。" onClose={close} onConfirm={() => resultMessage(addSecondary(team.id, { ...secondaryForm, requireReview: false }), onToast, close)}>
      ...
      <Alert title="唯一性检查">同一代理同一站点、币种和周期只能进入一个团队或单线代理。</Alert>
    </Modal>
    ...
    <Modal open={modal === 'detail'} title={`${team?.name || ''} · 详情与设置`} description="查看本站团队组织、线路业绩和代理类型。" onClose={close} onConfirm={savePreferences} confirmText="保存团队设置" width={1040}>
      {team && <div className="ta-stack"><MetricGrid columns={2}><MetricCard label="团队当前余额" value={<Money value={team.metrics.correctedNet} signed />} helper="当月结余 = 冲正后净输赢" tone="orange" /><MetricCard label="未结算收益" value={<Money value={team.metrics.payable} />} helper={`${team.metrics.grade} / ${(team.metrics.rate * 100).toFixed(0)}% 团队返佣`} tone="green" /></MetricGrid><DescriptionGrid columns={4} items={[{ label: '团队负责人', value: team.mainAgent }, { label: '代理类型', value: getTeamType(team) }, { label: '推荐人', value: recommenderOf(team) }, { label: '创建时间', value: team.createdAt || '—' }, { label: '团队方案', value: team.plan }, { label: '生效周期', value: team.startCycle }, { label: '团队状态', value: <StatusTag>{team.status}</StatusTag> }, { label: '团队代理总人数', value: overviewCounts.agentTotal }, { label: '总会员数', value: overviewCounts.memberTotal }, { label: '活跃会员数', value: overviewCounts.activeMembers }, { label: '副线', value: overviewCounts.secondaryTotal }, { label: '单线代理', value: overviewCounts.singleTotal }, { label: '团队返佣等级', value: `${team.metrics.grade} / ${(team.metrics.rate * 100).toFixed(0)}%` }]} /><TeamGradeSummary metrics={team.metrics} progress={gradeProgress} /><FormGrid><Field label="代理类型"><Select value={preferenceForm.teamType} onChange={(value) => setPreferenceForm({ ...preferenceForm, teamType: value })} options={['官方代理', '普通代理']} /></Field></FormGrid><Alert title="当月结余归属规则">加入团队当月结余带入，移出团队留在原团队；解散团队时由站点指定代理承担。</Alert><DataTable minWidth={1180} columns={[{ key: 'identity', label: '身份', render: (value) => <StatusTag tone="blue">{value}</StatusTag> }, { key: 'lineId', label: 'line_id' }, { key: 'agent', label: '代理名称' }, { key: 'activeMembers', label: '活跃会员', render: (value, row) => <Link disabled={!Number(value)} onClick={() => setModal(`lineMembers:${row.lineId}:activeMembers`)}>{value}</Link> }, { key: 'newActive', label: '新增活跃', render: (value, row) => <Link disabled={!Number(value)} onClick={() => setModal(`lineMembers:${row.lineId}:newActive`)}>{value}</Link> }, { key: 'firstDepositCount', label: '新增首存', render: (value) => Number(value || 0) }, { key: 'firstDepositAmount', label: '首存额度', render: (value) => <Money value={value} /> }, { key: 'netWinLoss', label: '净输赢值', render: (value) => <Money value={value} signed /> }, { key: 'status', label: '状态', render: (value) => <StatusTag>{value}</StatusTag> }]} rows={team.lines} rowKey="lineId" /></div>}
    </Modal>
...
function SiteSinglesPage({ onToast }) {
  ...
  const columns = [
    { key: 'code', label: '单线编号' }, { key: 'name', label: '单线代理', render: (value) => <b className="ta-primary-text">{value}</b> }, { key: 'owner', label: '单线代理' }, { key: 'source', label: '创建来源' }, { key: 'recommender', label: '上级代理' }, { key: 'plan', label: '佣金方案' },
    ...
    { key: 'action', label: '操作', render: (_, row) => <div className="ta-table-actions"><Link onClick={() => onToast(`${row.name} 详情已打开`)}>详情</Link><Link onClick={() => resultMessage(requestChange({ type: '单线代理加入团队', applicant: row.owner, currentUnit: row.name, targetUnit: 'apppay01部 / 待分配 line_id' }), onToast)}>加入团队</Link><Link onClick={() => resultMessage(requestChange({ type: '终止单线代理', applicant: row.owner, currentUnit: row.name, targetUnit: '终止', recommender: row.recommender }), onToast)}>终止</Link></div> },
  ]
  return <>
    <SectionHeader title="单线代理管理" description="按本站范围直接创建单人单线，或查看由团队副线切换形成的单线代理。" actions={<Button icon={<PlusOutlined />} onClick={() => setShowCreate(true)}>创建单人单线</Button>} />
    ...
    <FilterBar onSearch={() => onToast('单线代理列表已查询')} onReset={() => onToast('筛选条件已重置')}><Field label="单线代理"><Input placeholder="代理账号" /></Field><Field label="来源"><Select value="" placeholder="全部来源" options={['站点直接创建', '副线转单线代理']} /></Field><Field label="状态"><Select value="" placeholder="全部状态" options={['待生效', '生效中', '冻结', '已终止']} /></Field></FilterBar>
    <DataTable paginated minWidth={1250} columns={columns} rows={data.singles} />
    <Modal open={showCreate} title="创建单人单线" description="初始业务范围只包含该单线代理本人节点。" onClose={() => setShowCreate(false)} onConfirm={() => resultMessage(createSingle(form), onToast, () => setShowCreate(false))}>
      <FormGrid><Field label="单线名称"><Input value={form.name} onChange={(value) => setForm({ ...form, name: value })} /></Field><Field label="单线代理" required><Input value={form.owner} onChange={(value) => setForm({ ...form, owner: value })} /></Field><Field label="佣金方案"><Select value={form.plan} onChange={(value) => setForm({ ...form, plan: value })} options={data.plans.filter((plan) => plan.type === '单线代理方案').map((plan) => plan.name)} /></Field><Field label="推荐主线"><Input value={form.recommender} onChange={(value) => setForm({ ...form, recommender: value })} placeholder="可不填写" /></Field><Field label="生效周期"><Select value={form.startCycle} onChange={(value) => setForm({ ...form, startCycle: value })} options={['2026-08', '2026-09']} /></Field></FormGrid>
    </Modal>
  </>
}
...
function SiteReviewPage({ onToast }) {
  ...
  const columns = [
    ...
    { key: 'recommender', label: '上级代理' }, { key: 'conflict', label: '冲突检查', render: (value) => <StatusTag>{value}</StatusTag> }, { key: 'status', label: '状态', render: (value) => <StatusTag>{value}</StatusTag> },
  ...
  return <>
    <SectionHeader title="模式变更审核" description="审核副线转单线代理、单线代理加入团队、换主线和其他未来周期关系变更。" />
    <MetricGrid columns={4}><MetricCard label="待处理申请" value={data.requests.filter((item) => ['待站点复核', '待补充资料'].includes(item.status)).length} tone="orange" icon={<AuditOutlined />} /><MetricCard label="副线转单线代理" value={data.requests.filter((item) => item.type === '副线转单线代理').length} tone="blue" /><MetricCard label="加入团队" value={data.requests.filter((item) => item.type === '单线代理加入团队').length} /><MetricCard label="存在阻止项" value={data.requests.filter((item) => !item.conflict.startsWith('无')).length} tone="red" /></MetricGrid>
    <FilterBar onSearch={() => onToast('审核列表已查询')} onReset={() => setStatus('待处理')}><Field label="处理状态"><Select value={status} onChange={setStatus} options={['待处理', '全部', '已批准·待生效', '审核退回']} /></Field><Field label="申请类型"><Select value="" placeholder="全部类型" options={['副线转单线代理', '单线代理加入团队', '团队换主线', '终止单线代理']} /></Field><Field label="生效周期"><Select value="" placeholder="全部周期" options={['2026-08', '2026-09']} /></Field></FilterBar>
    ...
    <Modal open={!!selected} title={`${selected?.id || ''} · 审核详情`} description={selected?.type} onClose={() => setSelected(null)} onConfirm={() => { const result = reviewRequest(selected.id, true); resultMessage(result, onToast, () => setSelected(null)) }} confirmText="审核通过">
      {selected && <div className="ta-stack"><DescriptionGrid columns={2} items={[{ label: '申请人', value: selected.applicant }, { label: '目标周期', value: selected.effectiveCycle }, { label: '原结算单元', value: selected.currentUnit }, { label: '目标结算单元', value: selected.targetUnit }, { label: '上级代理', value: selected.recommender }, { label: '当月结余处理', value: selected.balanceHandling }, { label: '冲突检查', value: <StatusTag>{selected.conflict}</StatusTag> }]} /><Alert tone={selected.conflict === '无冲突' ? 'info' : 'error'} title={selected.conflict === '无冲突' ? '检查通过' : '存在阻止项'}>{selected.note}</Alert></div>}
    </Modal>
  </>
}
...
function SitePlansPage({ onToast }) {
  ...
  const configs = [
    ...
    { key: 'single', title: '单线代理方案', value: active.single, options: data.plans.filter((plan) => plan.type === '单线代理方案').map((plan) => plan.name), desc: '每条单线代理独立考核、独立定级。' },
  ...
  return <>
    <SectionHeader title="佣金方案" description="站点只配置本站适用的团队、单线代理和推荐奖励方案，并查看活跃定义与代理成本口径。" actions={tab === 'config' ? <Button onClick={saveConfig}>保存设置</Button> : null} />
...
function SiteSettlementPage({ onToast }) {
  ...
  const scopedBills = data.bills.filter((bill) => bill.site === SITE_NAME).map((bill) => ({
    ...bill,
    displayAgentType: ['团队佣金', '单线代理佣金'].includes(bill.type) ? '团队代理' : (bill.agentType || '普通代理'),
    displayIdentity: bill.type === '团队佣金' ? '团队负责人' : bill.type === '单线代理佣金' ? '单线代理' : '推荐代理',
  }))
  ...
  return <>
    <SectionHeader title="代理佣金结算" description="站点只处理本站团队、单线代理和推荐奖励账单，并在审核通过后发起发放。" />
    ...
    <FilterBar onSearch={() => onToast(`已查询 ${rows.length} 条本站代理账单`)} onReset={() => setFilters(emptyFilters)}><Field label="账单类型"><Select value={filters.billType} onChange={(billType) => setFilters({ ...filters, billType })} placeholder="全部类型" options={['团队佣金', '单线代理佣金', '推荐奖励']} /></Field><Field label="代理类型"><Select value={filters.agentType} onChange={(agentType) => setFilters({ ...filters, agentType })} placeholder="全部类型" options={['团队代理', '普通代理']} /></Field><Field label="状态"><Select value={filters.state} onChange={(state) => setFilters({ ...filters, state })} placeholder="全部状态" options={['待提交', '待审核', '待发放', '部分发放', '已发放']} /></Field><Field label="收款方"><Input value={filters.payee} onChange={(payee) => setFilters({ ...filters, payee })} placeholder="代理账号" /></Field></FilterBar>
