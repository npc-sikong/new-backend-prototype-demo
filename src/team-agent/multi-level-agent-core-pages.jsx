...
export function MultiLevelDashboardPage({ onToast }) {
  ...
  return <section className="ml-screen ml-dashboard-screen">
    ...
    <Alert title="数据说明">这里展示整个站点的代理、一般为累计数据，只有在代理新增数据时会每天更新；不建议通过日期筛选来更新，白底卡片数据会根据日期筛选范围同步变化。</Alert>
    {DASHBOARD_GROUPS.map((group) => <section className="ml-dashboard-group" key={group.title}>
      <h2>{group.title}</h2>
