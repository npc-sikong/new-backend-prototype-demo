...
function DownlinePage({ role, onToast, onNavigate }) {
  ...
  const columns = [
    ...
    { key: 'model', label: '代理模型' }, { key: 'settlementMode', label: '结算模式' }, { key: 'identity', label: '结算身份', render: (value) => <StatusTag>{value}</StatusTag> },
  ...
  return <>
    <SectionHeader title="下级代理" description={`${meta.label} ${meta.account} 的当前代理树、经营归属和关系版本。`} actions={<Button icon={<SwapOutlined />} variant="ghost" onClick={() => onNavigate?.('requests')}>关系与模式申请</Button>} />
    ...
    <FilterBar onSearch={() => onToast?.(`已查询到 ${rows.length} 条代理记录`)} onReset={() => setFilters({ keyword: '', status: '', model: '' })} onExport={() => onToast?.(`已生成 ${rows.length} 条下级代理导出演示`)}>
      ...
      <Field label="结算身份"><Input value={meta.identity} disabled /></Field>
    ...
    <Modal open={!!selected} title={`${selected?.account || ''} · 代理详情`} description="关系变更历史保留在代理详情页签，不另建重复菜单。" onClose={() => setSelected(null)} onConfirm={() => setSelected(null)} confirmText="关闭" showCancel={false} width={980}>
      ...
      {selected && tab === 'base' && <DescriptionGrid columns={3} items={[
        ...
        { label: '上级代理', value: selected.parent }, { label: '结算模式', value: selected.settlementMode }, { label: '结算身份', value: selected.identity },
...
function ReadonlyPlansPage({ role }) {
  ...
  const [selected, setSelected] = useState(null)
  const columns = [
    { key: 'id', label: '方案编号' }, { key: 'name', label: '方案名称', render: (value) => <b className="ta-primary-text">{value}</b> }, { key: 'type', label: '方案类型' }, { key: 'site', label: '所属站点' },
  ...
  return <>
    <SectionHeader title="返佣方案" description={`${meta.account} 当前结算身份可查看的站点生效方案；代理端为只读展示。`} />
    <Alert title="只读边界" tone="warning">方案调整由站点后台完成，变更仅影响未来完整周期；当前及历史账单继续使用生成时的方案快照。</Alert>
    ...
    <Modal open={!!selected} title={`${selected?.name || ''} · 方案详情`} description="查看当前方案等级门槛或推荐奖励口径。" onClose={() => setSelected(null)} onConfirm={() => setSelected(null)} confirmText="关闭" showCancel={false} width={900}>
      {selected?.type === '推荐奖励方案' ? <DescriptionGrid columns={2} items={[{ label: '奖励基数', value: selected.rewardBase }, { label: '奖励比例', value: <Percent value={selected.rewardRate} /> }, { label: '是否扣减线主佣金', value: selected.deductedFromSingle ? '是' : '否' }, { label: '生效周期', value: selected.effectiveCycle }]} /> : <DataTable columns={[
        { key: 'grade', label: '等级' }, { key: 'newActive', label: '新增活跃会员' }, { key: 'activeMembers', label: '活跃会员' }, { key: 'netWinLoss', label: '当月结余门槛', render: (value) => <Money value={value} /> }, { key: 'rate', label: '佣金比例', render: (value) => <Percent value={value} /> },
