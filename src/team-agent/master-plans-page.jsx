...
import { Button, DataTable, Field, FormGrid, Input, Modal, Money, Percent, SectionHeader, Select, StatusTag, Tabs, Toolbar } from './ui'
...
const REBATE_UPDATED_AT = '2026-07-17 19:12'
...
const REWARD_SITE_OPTIONS = ['旺财体育', '财神客栈', '全部站点']
const REWARD_STATUS_OPTIONS = ['生效中', '待生效', '停用']
const EMPTY_REWARD_FORM = { name: '', site: '旺财体育', netProfit: 0, rewardPercent: 10, effectiveCycle: '2026-08', status: '待生效', rewardBase: '推荐人员净收益' }

function buildRewardRows(plans, bills) {
  const rewardRows = plans.filter((plan) => plan.type === '推荐奖励方案').map((plan, index) => {
    const bill = bills.find((item) => item.type === '推荐奖励' && item.site === plan.site)
    const netProfit = Number(plan.netProfit ?? bill?.netWinLoss ?? bill?.correctedNet ?? bill?.totalWinLoss ?? 0)
    return { id: plan.id, sequence: index + 1, name: plan.name, site: plan.site || '全部站点', netProfit, rewardRate: Number(plan.rewardRate || 0), rewardBase: '推荐人员净收益', effectiveCycle: plan.effectiveCycle || '2026-08', status: plan.status || '待生效', operator: 'codex', operatedAt: REBATE_UPDATED_AT }
  })
  const demoRows = [
    { id: 'PLAN-R-002', name: '财神推荐奖励8%方案', site: '财神客栈', netProfit: 42000, rewardRate: 0.08, rewardBase: '推荐人员净收益', effectiveCycle: '2026-08', status: '待生效', operator: 'codex', operatedAt: REBATE_UPDATED_AT },
    { id: 'PLAN-R-003', name: '全站推荐奖励6%方案', site: '全部站点', netProfit: 120000, rewardRate: 0.06, rewardBase: '推荐人员净收益', effectiveCycle: '2026-09', status: '待生效', operator: 'codex', operatedAt: REBATE_UPDATED_AT },
  ]
  return [...rewardRows, ...demoRows].map((row, index) => ({ ...row, sequence: index + 1 }))
}

function rewardFormFromRow(row) {
  return { name: row.name, site: row.site, netProfit: row.netProfit, rewardPercent: Number((Number(row.rewardRate || 0) * 100).toFixed(2)), effectiveCycle: row.effectiveCycle, status: row.status, rewardBase: row.rewardBase || '推荐人员净收益' }
}
...
export function MasterPlansPage({ onToast }) {
  ...
  const [rewardRows, setRewardRows] = useState(() => buildRewardRows(data.plans, data.bills))
  ...
  const openRewardCreate = () => { setRewardMode('create'); setRewardTargetId(''); setRewardForm(EMPTY_REWARD_FORM) }
  const openRewardEdit = (row) => { setRewardMode('edit'); setRewardTargetId(row.id); setRewardForm(rewardFormFromRow(row)) }
  const saveReward = () => {
    if (!rewardForm.name.trim()) return onToast?.('请输入推荐奖励方案名称', 'error')
    const row = { id: rewardTargetId || `PLAN-R-${Date.now()}`, name: rewardForm.name, site: rewardForm.site, netProfit: Number(rewardForm.netProfit || 0), rewardRate: Number(rewardForm.rewardPercent || 0) / 100, rewardBase: rewardForm.rewardBase || '推荐人员净收益', effectiveCycle: rewardForm.effectiveCycle, status: rewardForm.status, operator: 'codex', operatedAt: REBATE_UPDATED_AT }
    ...
    setRewardMode(''); onToast?.(rewardMode === 'edit' ? '推荐奖励方案已修改' : '推荐奖励方案已新增', 'success')
  }
  const removeReward = (row) => { setRewardRows((current) => current.filter((item) => item.id !== row.id).map((item, index) => ({ ...item, sequence: index + 1 }))); onToast?.('推荐奖励方案已删除', 'success') }
  ...
  const rewardColumns = [
    { key: 'sequence', label: '序号', cellClassName: 'legacy-rebate-index' }, { key: 'name', label: '推荐奖励方案名称' }, { key: 'site', label: '适用站点' },
    { key: 'netProfit', label: '推荐人员净收益', render: (value) => <Money value={value} /> }, { key: 'rewardRate', label: '推荐人提成百分比', render: (value) => <Percent value={value} /> },
    { key: 'payable', label: '预估推荐奖励', render: (_, row) => <Money value={Number(row.netProfit || 0) * Number(row.rewardRate || 0)} /> }, { key: 'effectiveCycle', label: '生效周期' },
    { key: 'status', label: '状态', render: (value) => <StatusTag>{value}</StatusTag> },
    { key: 'action', label: '操作', render: (_, row) => <div className="ta-table-actions"><ActionLink onClick={() => openRewardEdit(row)}><EditOutlined /> 修改</ActionLink><ActionLink onClick={() => removeReward(row)}><DeleteOutlined /> 删除</ActionLink></div> },
  ]
  ...
  return <>
    <section className="legacy-rebate-screen">
      <SectionHeader title="佣金方案" description="按原返佣方案列表维护代理返佣配置，并通过推荐奖励切页维护不同站点的推荐奖励方案。" />
      ...
      {tab === 'reward' && <><Toolbar><Button icon={<PlusOutlined />} onClick={openRewardCreate}>新增推荐奖励</Button><Button icon={<DownloadOutlined />} variant="warning" onClick={() => onToast?.('推荐奖励方案已导出', 'success')}>导出</Button></Toolbar><DataTable minWidth={1120} columns={rewardColumns} rows={rewardRows} className="legacy-rebate-table" rowKey="id" paginated /></>}
    </section>
    ...
    <Modal open={!!rewardMode} title={rewardMode === 'edit' ? '修改推荐奖励' : '新增推荐奖励'} description="可按站点维护多套推荐奖励方案，推荐奖励按推荐人员净收益乘提成百分比预估。" onClose={() => setRewardMode('')} onConfirm={saveReward} width={760}>
      <FormGrid><Field label="方案名称" required><Input value={rewardForm.name} onChange={(name) => setRewardForm({ ...rewardForm, name })} placeholder="请输入推荐奖励方案名称" /></Field><Field label="适用站点"><Select value={rewardForm.site} onChange={(site) => setRewardForm({ ...rewardForm, site })} options={REWARD_SITE_OPTIONS} /></Field><Field label="推荐人员净收益"><Input type="number" min="0" value={rewardForm.netProfit} onChange={(netProfit) => setRewardForm({ ...rewardForm, netProfit })} /></Field><Field label="推荐人提成百分比"><Input type="number" min="0" max="100" step="0.01" value={rewardForm.rewardPercent} onChange={(rewardPercent) => setRewardForm({ ...rewardForm, rewardPercent })} /></Field><Field label="生效周期"><Select value={rewardForm.effectiveCycle} onChange={(effectiveCycle) => setRewardForm({ ...rewardForm, effectiveCycle })} options={['2026-07', '2026-08', '2026-09']} /></Field><Field label="状态"><Select value={rewardForm.status} onChange={(status) => setRewardForm({ ...rewardForm, status })} options={REWARD_STATUS_OPTIONS} /></Field></FormGrid>
    </Modal>
  </>
}
