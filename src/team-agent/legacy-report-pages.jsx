...
function buildCommissionRecords(data) {
  return buildFinance(data).map((item, index) => ({
    ...item, id: `COM-${String(index + 1).padStart(3, '0')}`, billNo: item.recordNo, type: item.identity === '副线负责人' ? '主线内部结算' : item.identity === '独立线主' ? '独立单线佣金' : item.identity === '主管主线' ? '团队佣金' : '历史代理佣金',
...
function detailAlert(page, detail) {
  if (page === 'members') return <Alert title="会员归属说明">会员按当前生效线路归集；主管主线可查看 gaodashang 团队代理树，副线负责人仅查看 WC002 本人线路，独立线主仅查看 dailiwc001 独立线。</Alert>
