...
function CommissionBasis({ data, role }) {
  ...
  const fallbackPlan = data.plans.find((item) => item.status === '生效中' && item.type === (role === 'independent' ? '单线代理方案' : '团队佣金方案'))
...
function MainDashboard({ onToast }) {
  ...
  return <>
    <SectionHeader title="团队经营看板" description={`${team.name} · 当前主线 ${team.mainAgent} · 团队合并考核`} actions={<><Button icon={<PlusOutlined />} onClick={() => setModal('secondary')}>开设副线</Button><Button icon={<WalletOutlined />} variant="success" onClick={() => setModal('settle')}>副线结算</Button><Button icon={<SwapOutlined />} variant="ghost" onClick={() => setModal('transfer')}>转单线代理</Button></>} />
    ...
    <FormulaPanel title="当月结余与佣金口径" items={[{ label: '净输赢', formula: '总输赢 − 场馆费 − 会员红利 − 会员返水 + 账户调整 + 补单输赢 − 存款手续费 − 提款手续费' }, { label: '冲正后净输赢 / 本月结余', formula: '净输赢 + 上周期结余 + 本月结余调整', value: `¥${team.metrics.correctedNet.toLocaleString()}` }, { label: '平台应付佣金', formula: 'MAX（0，冲正后净输赢 × 佣金比例 + 佣金调整）', value: `¥${team.metrics.payable.toLocaleString()}` }]} />
    ...
    <Modal open={modal === 'transfer'} title="副线转单线代理" description="当前周期继续并入团队，下一完整周期开始独立考核。" onClose={close} onConfirm={() => show(requestChange({ type: '副线转单线代理', applicant: transfer.agent, currentUnit: `${team.name} / ${team.lines.find((line) => line.agent === transfer.agent)?.lineId || '待确认'}`, targetUnit: `${transfer.agent} 单线代理`, effectiveCycle: transfer.effectiveCycle, recommender: team.mainAgent }), onToast, close)}>
      <FormGrid><Field label="目标副线"><Select value={transfer.agent} onChange={(value) => setTransfer({ ...transfer, agent: value })} options={team.lines.filter((line) => line.identity === '副线').map((line) => line.agent)} /></Field><Field label="生效周期"><Select value={transfer.effectiveCycle} onChange={(value) => setTransfer({ ...transfer, effectiveCycle: value })} options={['2026-08', '2026-09']} /></Field><Field label="默认上级代理"><Input value={team.mainAgent} disabled /></Field></FormGrid>
    </Modal>
  </>
}
...
function SecondaryDashboard({ onToast }) {
  ...
  const request = data.requests.find((item) => item.applicant === 'WC002' && item.type === '副线转单线代理')
  return <>
    <SectionHeader title="我的副线经营" description={`${team.name} / ${line.lineId} · 当前周期并入团队统一考核`} actions={<Button icon={<SwapOutlined />} onClick={() => show(requestChange({ type: '副线转单线代理', applicant: 'WC002', currentUnit: `${team.name} / ${line.lineId}`, targetUnit: 'WC002 单线代理', recommender: team.mainAgent }), onToast)}>申请转单线代理</Button>} />
...
function IndependentDashboard({ onToast }) {
  ...
  const bill = data.bills.find((item) => item.unitId === single.id && item.type === '单线代理佣金')
  const reward = data.bills.find((item) => item.unitId === single.id && item.type === '推荐奖励')
  return <>
    <SectionHeader title="单线代理经营看板" description={`${single.name} · 独立考核、独立定级、平台直接结算`} actions={<><Button icon={<TeamOutlined />} onClick={() => show(requestChange({ type: '单线代理加入团队', applicant: single.owner, currentUnit: single.name, targetUnit: 'apppay01部 / 待分配 line_id' }), onToast)}>申请加入团队</Button><Button variant="ghost" onClick={() => show(requestChange({ type: '终止单线代理', applicant: single.owner, currentUnit: single.name, targetUnit: '终止', recommender: single.recommender }), onToast)}>申请终止</Button></>} />
    <MetricGrid columns={4}><MetricCard label="当前等级" value={single.metrics.grade} helper={`${(single.metrics.rate * 100).toFixed(0)}% 佣金比例`} tone="blue" /><MetricCard label="冲正后净输赢" value={<Money value={bill?.correctedNet || single.metrics.assessmentNet} />} helper="即本月结余" tone="green" /><MetricCard label="平台应付佣金" value={<Money value={bill?.payable || 0} />} helper="直接发给单线代理" icon={<WalletOutlined />} /><MetricCard label="账单状态" value={bill?.state || '待生成'} helper={`已发 ¥${(bill?.issued || 0).toLocaleString()}`} tone="orange" /></MetricGrid>
    ...
    <FormulaPanel title="单线代理与推荐奖励" items={[{ label: '冲正后净输赢 / 本月结余', formula: '净输赢 + 上周期结余 + 本月结余调整', value: `¥${(bill?.correctedNet || single.metrics.assessmentNet).toLocaleString()}` }, { label: '单线代理应付佣金', formula: 'MAX（0，冲正后净输赢 × 40% + 佣金调整）', value: `¥${single.metrics.payable.toLocaleString()}` }, { label: '推荐奖励', formula: '已审核应付佣金 × 10%', value: `¥${(reward?.payable || 0).toLocaleString()}` }]} warning="单线代理加入团队时，当月结余随代理带入新团队；从加入生效周期停止推荐奖励，历史账单保持不变。" />
  </>
}
...
function AgentInternal({ role, onToast }) {
  ...
  if (role === 'independent') return <><SectionHeader title="副线内部结算" description="单线代理由平台直接结算，不参与团队副线内部结算。" /><EmptyState title="当前身份不适用" description="如未来加入团队并成为副线，将在此查看主线向本人的结算记录。" /></>
...
function AgentRequests({ role, onToast }) {
  ...
  function createDefaultRequest() {
    ...
    if (role === 'secondary') return requestChange({ type: '副线转单线代理', applicant: account, currentUnit: 'gaodashang01部 / LINE-B', targetUnit: 'WC002 单线代理', recommender: 'gaodashang' })
    return requestChange({ type: '单线代理加入团队', applicant: account, currentUnit: '单线代理01', targetUnit: 'apppay01部 / 待分配 line_id' })
  }
  return <>
