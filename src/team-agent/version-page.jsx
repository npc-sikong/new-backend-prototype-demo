import { useState } from 'react'
import { ApartmentOutlined, BankOutlined, MobileOutlined, SafetyCertificateOutlined, TeamOutlined } from '@ant-design/icons'
import { Button, Tabs } from './ui'

const VERSION_2_GROUPS_BASE = [
  {
    portal: 'master', title: '总控后台', icon: <SafetyCertificateOutlined />, items: [
      ['agents', '代理列表', '代理基础资料与团队代理层级', '代理类型仅保留多层级代理、星级代理和团队代理。新增和修改团队代理时，代理层级只提供团队负责人和副线；需要按单线经营时，由团队负责人关闭“是否能开副线”。历史单线记录打开修改时显示为团队负责人且开关关闭。', '新增和修改代理层级下拉均无单线代理；关闭开副线的团队负责人，在总控、站点、代理、H5及相关佣金列表中统一显示为单线代理。'],
      ['negativeProfit', '负盈利代理佣金结算', '负盈利账单查看与发放处理宽表', '结算页只保留负盈利代理佣金结算和佣金记录切页，返佣方案拆分为独立左侧入口；结算宽表继续展示最新盈亏、运营费用、分摊和佣金公式。', '结算页不再出现方案切页；负盈利账单、团队展开、总计、发放操作和佣金记录保持可用。'],
      ['negativeProfitReport', '负盈利代理佣金报表', '跨日期负盈利佣金只读报表', '总控、站点和代理后台共用同一字段顺序、计算与模拟数据定义；站点仅固定旺财体育，代理仅按团队负责人、副线或单线代理身份收窄范围。H5代理后台继续复用桌面代理报表字段，只改变卡片、详情抽屉和横向核对排版。', '四端可核对相同完整字段、字段筛选、金额格式和计算结果；站点无跨站数据，代理与H5无越权数据，H5详情及横向核对不缺字段。'],
      ['teams', '团队代理管理', '团队列表、成员下钻与团队详情', '团队列表移除代理编号/账号筛选；团队详情去除团队业绩查看，只保留团队概况和代理操作记录。冻结团队时，团队负责人及全部副线代理均不能登录；解除冻结后恢复正常状态。', '团队列表不存在代理编号/账号筛选；详情仅有团队概况和代理操作记录；冻结确认弹窗明确登录影响，解除后团队和代理账号恢复正常。'],
      ['rebatePlans', '返佣方案', '层级与负盈利返佣配置', '列表增加新增入口；新增弹窗可切换层级返佣方案与负盈利返佣方案。层级方案按代理层级维护0～1返佣比例，负盈利方案继续维护扶持期与常规两套配置。', '可新增两类返佣方案；层级表支持添加、删除级别，负盈利方案完整保留扶持月数、等级条件和活跃判定。'],
      ['records', '佣金记录', '平台佣金发放记录', '佣金记录移除副线内部结算切页，仅保留平台账单记录。列表按代理类型、代理身份、代理层级顺序展示；代理身份仅为官方代理或普通代理，原团队负责人、副线、单线代理、星级和层级内容改在代理层级展示。', '可按代理账号、站点名称、代理类型和发放时间查询；列表与详情均可核对官方/普通代理身份、代理层级及佣金发放信息，不再出现副线内部结算切页。'],
      ['revenue', '代理收益看板', '代理收益明细宽表', '按截图重构为原后台收益看板样式，保留模块页签、页面说明、所属站点、代理账号、查询日期、导出数据、下载文件和横向收益宽表。', '可按站点、代理账号和日期范围查询；表头展示佣金、余额、充值提现、投注、福利、欠款、运营费、代理人数和会员人数等字段，无数据时显示暂无数据。'],
      ['cycle', '结算周期设置', '分代理类型的结算周期配置', '从代理列表切页拆出并恢复为代理管理下独立入口；结算类型调整为层级代理和负盈利代理，两类均可选择周结或月结；周结频率可选一周、二周或三周。', '可从独立菜单进入，切换层级代理/负盈利代理并分别配置周结/月结；选择周结后可配置一周、二周、三周频率、结算日与执行时间。'],
    ],
  },
  {
    portal: 'site', title: '站点后台', icon: <BankOutlined />, items: [
      ['siteDashboard', '运营首页', '站点代理运营总览', '保留原指标看板、周期切换、趋势和排行，增加佣金余额、代理余额、指标组成明细、结算归属说明和团队维度下钻。', '可切换周期、点击每张指标卡查看组成，并核对团队或结算单元归属。'],
      ['siteAgents', '代理列表', '本站代理主档与团队代理兼容', '同步总控同源页面名称，将原“代理管理”改为“代理列表”；在代理账号右侧展示官方代理/普通代理身份，原代理身份改名为代理层级，并保留代理类型、代理部、line_id、生效周期和当月结余字段；关闭开副线的团队负责人按单线代理展示。', '可筛选并查看本站代理资料；不能查看其他站点数据，且关闭开副线的团队负责人在代理层级中显示为单线代理。'],
      ['teams', '团队代理管理', '本站团队结构与副线下钻', '同步总控同名页面的团队人数、会员人数、团队副线/单线及明细弹窗，去除副线会员详情字段；副线即团队内成员身份，代表本人 line_id 范围，不作为单独负责人角色；团队详情补充团队当前余额、未结算收益、团队代理总人数、总会员数、活跃会员数、副线、单线代理、当前返佣星级、团队返佣等级、距离下一等级条件、新增首存和首存额度，并将当前返佣星级作为主体、下一等级条件作为副体。团队详情表同步使用代理名称、活跃会员、新增活跃、新增首存和首存额度，不展示业务范围，新增活跃和活跃会员可点击查看该代理直属会员。', '可创建本站代理部、开副线并点击人数查看对应代理或会员明细；团队详情优先核对本站当前返佣星级、团队余额、收益和人数结构，点击新增活跃或活跃会员只看当前代理直属会员，升档差距作为辅助信息，列表与详情不出现跨站点数据。'],
      ['review', '模式变更审核', '本站关系变更审核', '保留站点角色独有的关系审核页，不作为总控同名报表同步；继续审核副线转单线代理、单线代理加入团队和换主线。', '审核范围固定为本站，关系变化只影响未来完整周期。'],
      ['plans', '佣金方案', '本站佣金方案配置', '页面名称与总控统一，补充代理类型、新增活跃、活跃会员、总输赢、活跃会员判定条件和新增活跃判定条件；层级代理和星级代理仅展示返佣比例；仅维护本站生效方案。', '可维护旺财体育方案并核对等级条件，团队方案的活跃判定随方案生效，不能修改其他站点方案。'],
      ['commissionRecords', '佣金记录', '本站佣金发放查询', '同步总控同名报表的代理类型、代理身份和代理层级口径，去除跨站点字段和操作，不展示副线内部结算切页。', '仅查询旺财体育平台佣金记录，可筛选、分页、导出并查看官方/普通代理身份和代理层级详情。'],
      ['cycle', '结算周期设置', '本站代理结算周期', '恢复为代理管理下独立入口，当前站点固定为旺财体育；结算类型调整为层级代理和负盈利代理，周结频率可选一周、二周或三周。', '可分别保存本站层级代理和负盈利代理周结或月结设置；周结支持一周、二周、三周频率；该页不下发代理后台。'],
      ['members', '代理报表', '原有代理经营报表', '保留代理会员、代理财务、存款、游戏、账变、转账、充提统计、场馆费用和站点利润，统一增加团队、线路、结算身份、结算单元、生效周期及各页专属经营字段。', '各报表可按新增条件筛选、分页、导出和打开详情，金额页面展示统一当月结余口径。'],
      ['prepaid', '预付金账户', '本站预付金额度', '保留原预付金结构，增加团队归属、结算身份和结算单元。', '可调整本站代理预付金额度并查看资金明细。'],
      ['agentPay', '代理代存处理', '本站代理代存审核', '保留额度或佣金代存结构，增加团队归属、扣款账户、流水倍数和本站审核。', '可审核本站代理代存，结果同步相关存款、账变和转账记录。'],
    ],
  },
  {
    portal: 'agent', title: '代理后台', icon: <TeamOutlined />, items: [
      ['dashboard', '运营首页', '三身份代理经营总览', '保留原经营指标、周期切换和明细，增加团队负责人、副线、单线代理切换，以及团队线路、当月结余、佣金、余额、身份操作和数据隔离。副线即团队内成员身份，只查看本人 line_id 范围。', '三种身份切换可用；副线不显示其他线路、团队余额或平台应付账单。'],
      ['downline', '代理列表', '本人及授权下级代理', '页面名称与总控统一，将原“代理管理”改为“代理列表”；在代理账号右侧新增官方代理/普通代理身份，原代理身份改名为代理层级，并保留代理类型、结算单元、line_id和生效周期；关闭开副线的团队负责人按单线代理展示。', '团队负责人只看本人团队范围；代理层级根据开副线权限统一显示，关闭时列表与详情均显示单线代理。'],
      ['requests', '关系与模式申请', '代理本人关系申请', '保留代理角色独有的关系申请页，不作为总控同名报表同步。', '可提交当前身份允许的未来周期关系申请并查看处理记录。'],
      ['readonlyPlans', '佣金方案', '当前身份适用方案', '页面名称与总控统一，补充新增活跃、活跃会员、总输赢、活跃会员判定条件和新增活跃判定条件；层级代理和星级代理仅展示返佣比例；去除站点字段和修改操作。', '仅可只读查看当前身份适用方案及团队方案内的活跃判定规则。'],
      ['personalCommission', '个人佣金', '本人佣金汇总', '保留当前身份本人的佣金、到账、待到账和余额汇总；副线为团队内成员身份，仅查看本人 line_id 内部结算。', '主线与单线代理查看平台佣金，副线只查看内部结算到账。'],
      ['commissionRecords', '佣金记录', '本人佣金发放查询', '同步总控同名报表名称与身份口径，仅保留当前身份本人及授权下级范围。', '可筛选、分页、导出并查看本人权限内记录。'],
      ['internal', '副线内部结算', '团队内部结算', '保留团队负责人和副线角色独有页面，不作为总控同名报表同步；副线为团队内成员身份，只承接本人 line_id 范围的内部结算。', '团队负责人可向副线结算，副线只看本人收款，单线代理不适用。'],
      ['agentPay', '额度／佣金代存', '本人代存申请', '保留当前身份、结算单元、可选会员范围、扣款类型和流水倍数。', '只能向本人业务范围会员代存并校验本人余额。'],
      ['finance', '经营报表', '个人经营报表', '保留个人财务、存款、游戏、充提统计和场馆费用，统一增加团队、线路、结算身份、结算单元、生效周期筛选与详情，并按身份限制查看范围；副线为团队内成员身份，只看本人 line_id。', '筛选、分页、详情和导出可用；副线只看到本人 LINE-B，单线代理只看到单线代理01。'],
    ],
  },
]

const ROLE_SYNC_ITEMS = {
  site: [
    ['agents', '代理列表', '本站代理资料', '清理站点后台原代理菜单后，从总控代理列表按站点权限同步；固定旺财体育本站数据，隐藏所属站点字段和跨站点操作。', '只显示旺财体育代理资料，保留本站查询与详情，不可查看或操作其他站点。'],
    ['negativeProfit', '负盈利代理佣金结算', '本站负盈利账单查看与发放处理', '直接复用总控最新结算字段和佣金计算，固定为旺财体育；同步上周期盈亏、运营费用、成本分摊、佣金净收益、运营费用六项明细和全字段TIPS，并保留确认、不发放、修改发放及冻结代理名称/操作列。', '可处理本站负盈利账单并核对最新字段、费用明细与公式；团队拆分和总计一致，横向浏览时代理名称和操作始终可见。'],
    ['negativeProfitReport', '负盈利代理佣金报表', '本站负盈利佣金只读报表', '直接复用总控同名报表的完整字段、字段顺序、计算、筛选、团队展开和总计，只将根记录及成员明细固定为旺财体育本站，不提供结算操作。', '总控增加或调整同名报表字段后本站同步展示；本站列表、成员、总计和导出不包含其他站点数据。'],
    ['teams', '团队代理管理', '本站团队列表管理', '同步总控团队代理业务口径，仅保留本站团队列表、创建、维护、成员与会员下钻，不再内嵌团队详情。', '只管理旺财体育团队，点击团队详情可进入独立模块并定位对应团队。'],
    ['teamDetails', '团队详情', '本站团队集中详情', '团队详情仅保留团队概况和当前团队代理操作记录，不再展示团队业绩查看。', '切换团队后，团队概况和代理操作记录同步定位同一团队，只展示旺财体育本站范围。'],
    ['records', '佣金记录', '本站佣金发放查询', '同步总控佣金记录口径，仅展示旺财体育平台账单记录；代理类型右侧展示官方/普通代理身份，原身份字段改为代理层级。', '可筛选、分页、导出和查看本站记录详情，不出现副线内部结算切页。'],
    ['cycle', '结算周期设置', '本站结算周期配置', '结算周期设置仅下发站点后台并作为代理管理下独立入口，当前站点固定为旺财体育。', '可分别维护本站层级代理和负盈利代理结算周期。'],
  ],
  agent: [
    ['agents', '代理列表', '团队负责人及多层级代理的授权代理资料', '代理后台代理列表仅对团队负责人保留只读授权团队资料，多层级代理继续使用独立代理列表；副线和单线代理移除菜单与页面入口。所有含代理类型的现存列表和报表仍在其右侧展示推荐人。H5代理后台同步相同身份权限。', '团队负责人和多层级代理可进入各自代理列表；副线和单线代理切换后不显示该模块，并从原代理列表自动返回代理数据看板；总控与站点代理列表不受影响。'],
    ['negativeProfitReport', '负盈利代理佣金报表', '本人及推荐代理佣金只读报表', '直接复用总控与站点同名报表的字段、顺序、计算和数据定义，只按团队负责人、副线或单线代理身份收窄本人、团队及推荐代理范围；H5代理后台继续读取同一字段定义。', '桌面与H5完整字段一致；身份切换只改变可见记录和展开范围，不改变字段与计算；推荐数据不重复计入主记录总计。'],
    ['reversal', '冲正统计报表', '负盈利代理账期欠站点核对', '桌面代理与H5代理共用账期时间、代理名称、代理身份、欠站点、还站点和剩余欠款字段定义及欠款数据；团队负责人查看团队汇总，单线代理只看本人，副线无入口，多层级代理原冲正统计不变。', '桌面与H5均只按账期筛选且展示完整账期时间；六个字段顺序和金额一致，身份切换只改变数据范围。'],
  ],
}

const MERGED_ITEMS = {
  master: {
    agents: ['agents', '代理列表', '代理资料、推荐关系与取款密码', '新增和修改团队代理时增加推荐人选项；副线上级代理由所选团队负责人自动带出并锁定。代理列表及所有含代理类型的报表在代理类型右侧展示推荐人；原取款密码和日期筛选能力保持不变。', '团队代理可选择推荐人；副线指定团队后上级代理不可编辑；三后台含代理类型的报表均可在其右侧核对推荐人。'],
    negativeProfit: ['negativeProfit', '负盈利代理佣金结算', '负盈利账单与佣金记录', '结算页只保留负盈利代理佣金结算和佣金记录；返佣方案从本页拆出为独立菜单，结算字段、费用明细与公式保持不变。', '结算页不出现返佣方案切页；账单查询、发放处理和佣金记录均可正常使用。'],
    rebatePlans: ['rebatePlans', '返佣方案', '层级与负盈利返佣配置', '新增弹窗通过切页分别创建层级返佣方案和负盈利返佣方案；层级方案使用0～1返佣比例，负盈利方案保留扶持期与常规配置。', '新增按钮、两类方案切页、级别增删、扶持月数、两套负盈利配置及保存反馈均可演示。'],
    teams: ['teams', '团队代理管理', '团队列表与团队详情', '团队列表移除代理编号/账号筛选；团队详情仅保留团队概况和代理操作记录，去除团队业绩查看。冻结团队需二次确认，并明确团队负责人和全部副线代理均不能登录；解除后恢复正常状态。', '团队列表无代理编号/账号筛选；团队详情无团队业绩查看；冻结弹窗展示登录影响，确认后团队及成员账号冻结，解除后恢复正常。'],
  },
  site: {
    agents: ['agents', '代理列表', '本站代理资料、推荐关系与取款密码', '在旺财体育本站范围内同步团队代理推荐人选项；副线上级代理由所属团队负责人自动带出并锁定。含代理类型的报表在其右侧展示推荐人。', '可维护本站团队代理推荐人；副线上级代理不可编辑；本站报表可在代理类型右侧核对推荐人。'],
    negativeProfit: ['negativeProfit', '负盈利代理佣金结算', '本站负盈利账单与佣金记录', '本站同步总控最新结算字段、运营费用六项明细、全字段TIPS及佣金公式，去除旧人数、存提款、奖励明细、净输赢、结余和等级字段；佣金记录切页及原有发放功能保持不变。', '可在旺财体育范围核对新字段、费用明细和公式；团队成员拆分与主记录、底部总计一致。'],
    teams: ['teams', '团队代理管理', '本站团队列表与详情', '本站团队列表移除代理编号/账号筛选；团队详情仅保留团队概况和代理操作记录，去除团队业绩查看。冻结团队需二次确认，冻结期间团队负责人和全部副线代理不能登录；解除后恢复正常。', '旺财体育团队列表无代理编号/账号筛选；详情无团队业绩查看；冻结和解除操作可同步更新团队及成员代理账号状态。'],
  },
  agent: {
    teams: ['teams', '团队代理管理', '本人授权团队列表与详情', '团队列表移除代理编号/账号筛选；团队详情去除团队业绩查看，仅按当前身份展示授权团队概况和代理操作记录。', '团队负责人、副线和单线代理只能查看本人授权团队；桌面与H5详情均不再出现团队业绩查看。'],
  },
}

const REMOVED_AFTER_MERGE = {
  master: new Set(['plans', 'records', 'teamDetails']),
  site: new Set(['records', 'teamDetails']),
  agent: new Set(['teamDetails']),
}

const VERSION_2_GROUPS = VERSION_2_GROUPS_BASE.map((group) => {
  const source = ROLE_SYNC_ITEMS[group.portal] || group.items
  const items = source
    .filter((item) => !REMOVED_AFTER_MERGE[group.portal]?.has(item[0]))
    .map((item) => MERGED_ITEMS[group.portal]?.[item[0]] || item)
  const nextItems = group.portal === 'master' ? [
    ['negativeProfitModeGuide', '负盈利模式说明', '返佣定级、代理模式与结算口径说明', '总控后台在版本需求说明下新增独立业务说明页，并按最新口径展示负盈利模式的返佣升级条件、团队代理与单线代理规则、上周期结余及不发放或修改发放规则。页面同步展示代理佣金和当月净利润公式，并通过充值、真人场馆首充活动及彩票投注示例区分充值提现流水与场馆活动提现流水。', '菜单与页面跳转可用；页面可查看负盈利模式、团队代理、单线代理、结算模式四部分说明，两项佣金计算公式和四步提现流水示例；站点后台、代理后台和H5不出现该菜单。'],
    ['memberLockedFlow', '会员提现流水查询', '会员余额锁定与完整提现流水下钻', '主列表移除“未解锁彩金”，充值额度后直接展示总余额、可提现余额、锁定余额和充值/彩金提现流水。弹窗提现流水列表继续同时展示当前锁定中记录和历史已解锁记录、解锁时间，并提供当前会员完整提现流水CSV导出。', '主表不再出现未解锁彩金；member_10086明细仍可看到锁定中与已解锁状态、开始和解锁时间，历史记录还需解锁流水为0；点击导出可下载5条提现流水；站点后台与代理后台不出现本模块。'],
    ...items,
  ] : group.portal === 'agent' ? [
    ['h5Agent', 'H5代理后台', '代理登录、紧凑额度首页与H5资金操作', '新增独立代理登录页，按账号进入对应代理身份。首页当前可用额度使用宽幅蓝色卡片，并提供快速充值、余额提现、内部转账和发放红包四项资金操作；四项操作分别使用暗色H5底部抽屉，完整展示充值渠道与协议、提现方式与账户、转账对象与流水倍数、红包定时与有效期等字段，表单可纵向滚动且主操作按钮固定。首页额度卡和快捷操作高度较前版缩小约25%，并在下方展示当前身份的其它模块入口。底部导航统一为“首页 / 看板 / 财务 / 个人中心 / 更多”，所有一级页面的业务说明集中放入说明抽屉；代理数据看板使用蓝色强调底和普通深色底区分累计/当前状态指标与日期范围指标。', '登录、四种身份首页和四项资金操作可用；四个抽屉均在窄屏容器内无横向溢出；首页其它模块入口完整，底部第三项进入财务中心、第四项进入个人中心；看板两种暗色底区分清晰，字段、数据和交互不变。', 'h5Agent'],
    ['mlDashboard', '多层级代理后台模块', '多层级代理经营模块及四身份共用原有页面', '代理后台身份切换支持团队负责人、副线、单线代理和多层级代理。代理数据看板按身份显示适用指标：多层级代理将提现中佣金改为未结算佣金；团队负责人、副线和单线代理移除本期佣金预估或净收益及提现中佣金；副线和单线代理继续移除代理数据整组；四种身份将代理手续费支出统一改名为充提手续运营费。', '四种身份均可进入代理数据看板；多层级代理显示未结算佣金，团队负责人保留代理数据，副线和单线代理不显示代理数据整组，所有身份资金流水显示充提手续运营费。'],
    ...items,
  ] : items
  return { ...group, items: nextItems }
})

const VERSION_1_GROUPS = [
  { portal: 'master', title: '总控后台', icon: <SafetyCertificateOutlined />, items: [['h5', 'H5 前端切换与提现页', '后台到会员端演示入口', '在详情页顶部保留 H5 前端切换入口；H5 提现页按手机端窄屏样式展示，选择提现方式后展开账户、金额和资金密码，并在提现金额标题下方展示可提现、锁定及“解锁条件”弹层。解锁条件底部以两条编号说明提示流水同步延迟，以及充值或活动盈利金额在所有提现流水完成后一并解锁。', '可从后台进入 H5、以手机端比例查看钱包概览、切换隐藏无余额场馆、选择提现方式、查看可提/锁定金额，并在解锁条件弹层内按“类型 / 锁定额度 / 还需解锁流水”合并核对场馆流水、充值流水和彩金流水；弹层底部两条提示完整显示且不遮挡列表。', 'h5']] },
  { portal: 'site', title: '站点后台', icon: <BankOutlined />, items: [] },
  { portal: 'agent', title: '代理后台', icon: <TeamOutlined />, items: [] },
]

const JULY_17_PAGES = new Set(['teams', 'plans', 'settlement', 'records', 'commissionRecords', 'reversal', 'returns', 'revenue', 'relations', 'cycle', 'readonlyPlans', 'bills'])
const JULY_18_PAGES = new Set(['agents', 'negativeProfit', 'siteAgents', 'downline', 'teams', 'teamDetails'])
const JULY_20_PAGES = new Set(['memberLockedFlow'])
const JULY_21_PAGES = new Set(['negativeProfitModeGuide'])
const JULY_22_PAGES = new Set(['agents', 'plans', 'cycle', 'negativeProfitReport', 'reversal', 'mlDashboard', 'h5Agent'])
const JULY_23_PAGES = new Set(['agents', 'teams', 'siteAgents', 'downline', 'negativeProfit', 'negativeProfitReport', 'h5Agent'])
const JULY_24_PAGES = new Set(['teams', 'negativeProfit', 'rebatePlans', 'negativeProfitReport', 'reversal', 'h5Agent'])

function VersionGroup({ group, navigateTo }) {
  return <section className="ta-version-group"><header><div><i>{group.icon}</i><div><h2>{group.title}</h2><span>按模块展示最新需求说明</span></div></div><b>{group.items.length} 个模块</b></header>
    {group.items.length ? <div className="ta-version-modules">{group.items.map(([page, title, module, change, acceptance, targetPortal]) => <article className="ta-version-module" key={`${group.portal}-${page}`}>
      <div className="ta-version-module-head"><div><h3>{title}</h3><time>完成时间：{JULY_24_PAGES.has(page) ? '2026-07-24' : JULY_23_PAGES.has(page) ? '2026-07-23' : JULY_22_PAGES.has(page) ? '2026-07-22' : JULY_21_PAGES.has(page) ? '2026-07-21' : JULY_20_PAGES.has(page) ? '2026-07-20' : page === 'h5' ? '2026-07-16' : JULY_18_PAGES.has(page) ? '2026-07-18' : JULY_17_PAGES.has(page) ? '2026-07-17' : '2026-07-15'}</time></div><Button size="small" variant="ghost" onClick={() => navigateTo(targetPortal || group.portal, targetPortal ? undefined : page)}>前往页面</Button></div>
      <div className="ta-version-content"><div><b>模块说明</b><p>{module}</p></div><div><b>修改说明</b><p>{change}</p></div><div><b>功能验收</b><p>{acceptance}</p></div></div>
    </article>)}</div> : <div className="ta-version-empty">本版本该后台无新增业务模块。</div>}
  </section>
}

export function VersionRequirementsPage({ navigateTo }) {
  const [version, setVersion] = useState('2.0')
  const groups = version === '2.0' ? VERSION_2_GROUPS : VERSION_1_GROUPS
  return <div className="ta-version-page">
    <div className="ta-version-hero"><div><span>{version} 版本 · {version === '2.0' ? '原第 27 周需求' : '原第 26 周需求'}</span><h1>{version === '2.0' ? '业务运营与团队代理演示原型' : 'H5 提现与后台切换演示'}</h1><p>{version === '2.0' ? '总控后台保留会员提现流水查询；代理后台支持四种身份，并新增独立第五入口“H5代理后台”，以暗夜金融风将现有代理业务完整适配为移动端体验，原四门户与原 H5 前端保持不变。' : '保留后台到 H5 前端的切换入口，以及手机端比例的钱包概览、提现方式、取款账户和金额输入演示。'}</p></div><div className="ta-version-seal">{version === '2.0' ? <ApartmentOutlined /> : <MobileOutlined />}<strong>{version}</strong><span>{version === '2.0' ? 'P0 业务演示' : '需求归档'}</span></div></div>
    <Tabs items={[{ value: '2.0', label: '2.0 · 第 27 周' }, { value: '1.0', label: '1.0 · 第 26 周' }]} active={version} onChange={setVersion} />
    <div className="ta-version-groups">{groups.map((group) => <VersionGroup key={`${version}-${group.portal}`} group={group} navigateTo={navigateTo} />)}</div>
    {version === '2.0' && <section className="ta-version-roadmap"><h2>后续增强能力</h2><p>以下能力只作为后续路线图，不计入本次已完成验收：批量开副线、内部结算模板、主线自有资金提前结算、方案计算预演、阶梯奖励和历史余额移交。</p></section>}
  </div>
}
